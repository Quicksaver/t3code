  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const makeClient = (events: Queue.Queue<OrchestrationShellStreamItem>) =>
          ({
        ...
        const cache = Persistence.EnvironmentCacheStore.of({
          loadShell: () => Effect.succeed(Option.some(activeSnapshot)),
          saveShell: () => Effect.void,
          loadThread: () => Effect.succeed(Option.none()),
          saveThread: (_environmentId, snapshot) =>
            Ref.update(savedThreads, (threadIds) => [...threadIds, snapshot.thread.id]),
        ...
        yield* retainCachedThread(cache, TARGET.environmentId, threadId);
        ...
        yield* persistCachedThread(
          cache,
          TARGET.environmentId,
          { snapshotSequence: 1, thread: { id: threadId } as never },
