...
Generated after upstream merge commit `7369bfbdc`, with `origin/main` at `7b378ec6d` and `upstream/main` at `97e5cd3bf`. Forty-nine new upstream commits were merged since the previously recorded upstream ref `cdaba7fa8`, spanning `e29ad7604` (`Unify mobile typography tokens across the app (#3162)`) through `97e5cd3bf` (`[codex] align server auth Effect services (#3180)`). No new `origin/main` commits were available beyond the current fork-tracking origin ref `7b378ec6d`. After this merge and reconciliation, local `main` is 238 commits ahead and 0 commits behind `upstream/main`, and 60 commits ahead and 0 commits behind `origin/main`; the remaining fork delta against current `upstream/main` is 262 files changed, 36787 insertions, and 1051 deletions.

One local behavior is retired by this merge: the old `?diff=1` right-panel override is no longer carried in `ChatView`/`rightPanelStore` because upstream now owns diff opening through the dedicated diff panel state introduced with diff scope switching. Upstream added diff scope switching, provider update settings, environment-scoped settings by default, right-panel close-on-last-tab behavior, Work Log capitalization, shared model favorite buttons, clipped provider badge fixes, workspace RPC error preservation, hardened preview ownership, mobile typography tokens, EAS label gating, and a broad Effect service-module refactor across desktop, server, relay, client-runtime, source control, terminal, checkpointing, auth, cloud, diagnostics, telemetry, persistence, preview, project, and workspace infrastructure. These upstream changes are authoritative; the local VS Code host bootstrap, custom Version Control panel, subagent threading, wide conversation defaults, workspace skill loading, terminal action reuse work, and mobile EAS ownership remain preserved around them.

Concrete conflict notes from this merge:

- `apps/server/src/config.ts`, `apps/server/src/provider/ProviderSkillsLister.ts`, `apps/server/src/ws.ts`, `apps/server/src/server.ts`, and `apps/server/src/vscodeWorkspaceBootstrap/http.ts` keep the local VS Code host bootstrap, host MCP server injection, workspace-scoped Codex skill listing, and legacy auth compatibility routes while adapting them to upstream's root Effect service modules and new auth error taxonomy.
...
- `apps/server/src/vcs/VcsStatusBroadcaster.ts` keeps the local git-ignore-aware filesystem watcher for local status refresh while accepting upstream's exported `make` service constructor and remote poller changes; the Version Control panel still does not add an independent periodic fetch timer.
