...
The latest worktree pick pass also brought in terminal-backed project action candidate classification, source-control thread metadata routing extraction from `ChatView`, draft source-control metadata updates, structured metadata error handling, and stale source-control metadata error suppression. Earlier review ports remain part of the fork inventory: archived-action local-API fallback, archive grouping/search helper extraction and bulk-action hardening, colon-safe archive project grouping keys reused by the Archive panel UI, activity detail patch merging/deleted-file/MCP-row handling, basename-safe changed-file/diff matching, stable same-timestamp unkeyed tool-update ordering, terminal-backed project action readiness/probing hardening, source-control panel review fixes, stale branch worktree path filtering, rename-diff fallback suppression, VCS stream local-watcher retention before initial snapshots and for sibling worktree updates, cwd-scoped source-control working-tree enrichment, selected-file commit/merge argument hardening, interrupted branch-detail load suppression, Codex workspace-skill invalid-cwd normalization, VS Code persistence cleanup coverage, and stricter subagent interrupt turn targeting. These ports intentionally adapt the isolated worktree fixes onto current `main` instead of replaying whole split-branch trees, because several worktrees lack root-owned VS Code, provider-skill, source-control, and chat customizations.
