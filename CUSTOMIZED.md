...
Generated after upstream merge commit `f4409e95e`, with the current tracked main refs now recorded as `origin/main` at `0952f8f40` and `upstream/main` at `82a9bcc7f`. One hundred sixty new upstream commits were merged since the previously recorded upstream ref `cc69aef4d`, spanning `40d14647d` (`[codex] Structure catalog dependency resolution failures (#3298)`) through `82a9bcc7f` (`[codex] add session context to credential errors (#3349)`). After the follow-up fork reconciliation and worktree update, `origin/main` advanced beyond the earlier recorded fork ref `7b378ec6d` through `0952f8f40` (`Update fork merge notes with latest worktree status`), while `upstream/main` stayed at `82a9bcc7f`. Local `main` is currently ahead of `origin/main` only by the latest selective worktree-port changes and remains ahead of `upstream/main`.
...
The port retained only targeted branch-local fixes that were absent from `main`: command output extraction and merge hardening for blank-only streams, whitespace-only incremental chunks, split chunks, shorter completed/updated snapshots, and shorter single-line repeated-prefix snapshots; terminal-backed project action terminal id collision fixes, fallback action terminal labels such as `Action: build (2)`, and conservative busy detection when POSIX process-tree inspection is incomplete; subagent parent metadata propagation, parent-collab child-shell synthesis, child-stop no-root-fallback behavior, resumed-child parent block de-duplication, item/turn-aware live-status matching, hidden terminal ancestor traversal, and active route subagent sidebar path visibility; Version Control panel fixes for default compare refs, non-current diverged merge sync, discard failure surfacing, fallback rename parsing, merged staged-plus-unstaged row stats, and late-month relative dates; and related `SOURCE_CONTROL.md` and `SUBAGENTS.md` documentation refreshes. The workspace-scoped skill-loading, thread-detail subscription race, and VS Code extension worktrees had no additional production code to port at that point because their useful behavior was already represented on `main`. Do not replay split worktrees wholesale: several remaining diffs are stale branch shape or would regress newer `main` behavior.

A later PR-comment sweep across those worktrees produced additional review-hardening fixes that were selectively ported on top of `main`: VS Code backend readiness now treats malformed readiness bodies as terminal and keeps body reads under the attempt timeout, webview bridge injection escapes inline script terminators and cleans up disposables if rendering fails, MCP socket startup cleans up server/socket state after listen failure, workspace-bootstrap HTTP error status logging is bounded, VS Code packaging scripts surface spawn startup/signal failures and trim catalog inline comments, desktop backend advertisement errors keep structured operation/home context without exposing private cause text, strict project-action terminal readiness fails when the attach stream ends before input readiness, subagent work-entry rows keep hook order stable, and work-log/file-change handling preserves prefix-like output chunks while trimming Codex file-change kinds. A second comment sweep tightened those ports further: VS Code readiness JSON body reads are explicitly raced against the attempt abort signal, stale async VS Code webview renders cannot overwrite newer HTML or registrations, bridge JSON values escape script-parser sentinels before injection, desktop advertisement warning logs include bounded sanitized cause diagnostics while preserving structured operation/home context, shorter multiline command-output prefix snapshots replace instead of duplicating prefixes while one-character streamed chunks still append, and Codex file-change kinds are trimmed and lowercased before add-patch normalization. The Version Control panel's five-minute mounted remote-fetch timer remains intentional and should be preserved.

## Latest Worktree Update

Generated after updating active worktrees against `upstream/main` at `82a9bcc7f` and current `origin/main` at `076677ac6`. All active worktrees now contain both refs and were left clean after branch-local validation and browser smoke checks: `fix/codex-skills` at `326c867da`, `split/file-command-activity-boxes` at `81a576a39`, `split/subagent-threading-work` at `b786893b1`, `split/terminal-backed-project-actions` at `b1045aa36`, `fix/thread-detail-subscription-race` at `7a1c76de1`, `split/version-control-panel-work` at `fbb943b8d`, and `split/vscode-extension-work` at `2c4b7cc3d`.

Most branches are now tree-equivalent to `origin/main`, which already carries the useful workspace skill loading, subagent threading, thread-detail subscription reliability, terminal-backed action production behavior, and VS Code extension implementation. A follow-up selective port brought over the remaining useful branch-local changes: expanded tool details now keep the default cursor, command-output repeated-prefix regression coverage was added, and terminal manager tests use the current `TerminalManager.__testing` module shape. The Version Control panel's five-minute mounted remote-fetch timer is intentional and should be preserved alongside manual fetch, VCS stream, watcher, focus/visibility, and explicit-operation refresh paths. The only intentionally unported worktree delta after that follow-up is the `split/version-control-panel-work` mechanical `packages/client-runtime/src/state/vcs.ts` atom-label/order cleanup; it is diagnostics-only and should stay branch-local unless there is a concrete reason to normalize those labels on `main`.
