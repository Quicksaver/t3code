...
The fork is now merged through upstream `6245c547c`. This includes the prior right-panel inset restoration, Electron renderer startup fixes, preview automation live-owner stream migration, preview automation readiness/device-toolbar edge-case handling, and Grok ACP replay-idle load gate, plus the latest chat send-anchoring stabilization and native mobile composer revision-gated updates. The latest bounded merge conflict-prone files were `apps/web/src/components/chat/MessagesTimeline.tsx` and `apps/web/src/components/chat/MessagesTimeline.test.tsx`; preserve upstream's sent-message anchor callbacks, work-log group expansion model, mobile thread activity updates, and native composer revision guard while keeping the fork's workspace-aware provider-skill rendering, subagent child-thread navigation, command/file activity rows, full-width conversation/composer defaults, and fold-settling `maintainScrollAtEnd` suppression.

The latest worktree pick pass also brought in terminal-backed project action candidate classification, project-action terminal launch reservations, source-control thread metadata routing extraction from `ChatView`, draft source-control metadata updates, structured metadata error handling, stale source-control metadata error suppression, hook-lifetime source-control metadata sequencing, status-derived default-branch detection, and activity-row preview/keyboard follow-ups. Earlier review ports remain part of the fork inventory: archived-action local-API fallback, archive grouping/search helper extraction and bulk-action hardening, colon-safe archive project grouping keys reused by the Archive panel UI, activity detail patch merging/deleted-file/MCP-row handling, basename-safe changed-file/diff matching, stable same-timestamp unkeyed tool-update ordering, terminal-backed project action readiness/probing hardening, source-control panel review fixes, stale branch worktree path filtering, rename-diff fallback suppression, VCS stream local-watcher retention before initial snapshots and for sibling worktree updates, cwd-scoped source-control working-tree enrichment, selected-file commit/merge argument hardening, interrupted branch-detail load suppression, Codex workspace-skill invalid-cwd normalization, VS Code persistence cleanup coverage, and stricter subagent interrupt turn targeting. These ports intentionally adapt the isolated worktree fixes onto current `main` instead of replaying whole split-branch trees, because several worktrees lack root-owned VS Code, provider-skill, source-control, and chat customizations.
