...
Generated after the bounded worktree pick refresh against local code-pick ref `c2cc1225e` and `upstream/main` ref `a9b1190a1`. After the upstream merge and worktree pick refresh, the fork is 370 commits ahead and 0 commits behind `upstream/main`; the current fork diff against `upstream/main` touches 317 files with 48991 insertions and 2027 deletions.

## Latest Merge Impact

The fork is now merged through upstream `a9b1190a1`. This includes the prior right-panel inset restoration, Electron renderer startup fixes, preview automation live-owner stream migration, preview automation readiness/device-toolbar edge-case handling, Grok ACP replay-idle load gate, chat send-anchoring stabilization, native mobile composer revision-gated updates, preview browser stabilization, and upstream desktop-local WSL/Windows backend mode picker. The latest bounded merge conflict-prone files were `apps/desktop/src/backend/DesktopBackendManager.ts`, `apps/web/src/connection/platform.ts`, `apps/web/src/components/Sidebar.tsx`, `apps/web/src/components/ChatView.tsx`, and `apps/web/src/components/chat/MessagesTimeline.tsx`; preserve upstream's desktop backend pool, WSL bootstraps, secondary desktop-local registrations, and preview/browser updates while keeping the fork's VS Code host bootstrap, desktop backend advertisements, source-control/sidebar glue, terminal availability guards, command/file activity rows, full-width conversation/composer defaults, and fold-settling `maintainScrollAtEnd` suppression.

The latest worktree pick pass also brought in WSL backend routing/distro-path fixes, host-connected terminal action gating, the dedicated Archive settings panel and its action/review follow-ups, and shared changed-file/diff path matching in `diffRendering`. Earlier review ports remain part of the fork inventory: terminal-backed project action candidate classification, project-action terminal launch reservations, source-control thread metadata routing extraction from `ChatView`, draft source-control metadata updates, structured metadata error handling, stale source-control metadata error suppression, hook-lifetime source-control metadata sequencing, status-derived default-branch detection, activity-row preview/keyboard follow-ups, archived-action local-API fallback, archive grouping/search helper extraction and bulk-action hardening, colon-safe archive project grouping keys reused by the Archive panel UI, activity detail patch merging/deleted-file/MCP-row handling, basename-safe changed-file/diff matching, stable same-timestamp unkeyed tool-update ordering, terminal-backed project action readiness/probing hardening, source-control panel review fixes, stale branch worktree path filtering, rename-diff fallback suppression, VCS stream local-watcher retention before initial snapshots and for sibling worktree updates, cwd-scoped source-control working-tree enrichment, selected-file commit/merge argument hardening, interrupted branch-detail load suppression, Codex workspace-skill invalid-cwd normalization, VS Code persistence cleanup coverage, and stricter subagent interrupt turn targeting. These ports intentionally adapt the isolated worktree fixes onto current `main` instead of replaying whole split-branch trees, because several worktrees lack root-owned VS Code, provider-skill, source-control, and chat customizations.
