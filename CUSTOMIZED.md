...
Generated after the bounded worktree comment/pick refresh against local code-pick ref `72ef94989` and `upstream/main` ref `7b9eef7ac`. After the upstream merge and worktree pick refresh, the fork is 415 commits ahead and 0 commits behind `upstream/main`; the current fork diff against `upstream/main` touches 318 files with 50647 insertions and 2116 deletions.

## Latest Merge Impact

The fork is now merged through upstream `7b9eef7ac`. This includes the prior right-panel inset restoration, Electron renderer startup fixes, preview automation live-owner stream migration, preview automation readiness/device-toolbar edge-case handling, Grok ACP replay-idle load gate, chat send-anchoring stabilization, native mobile composer revision-gated updates, preview browser stabilization, upstream desktop-local WSL/Windows backend mode picker, restored chat scroll affordances, the timeline minimap, timeline scroll anchoring helpers/tests, v0.0.28 package metadata, right-panel tab middle-click close, WSL distro pre-warm before WSL-only backend preflight, Claude Sonnet 5 defaults/aliases, and restored ultrathink frame border styling. The latest bounded merge conflict-prone files were `apps/web/src/components/BranchToolbar.tsx`, `apps/web/src/components/ChatView.tsx`, `apps/web/src/components/chat/ChatComposer.tsx`, `apps/web/src/components/chat/ComposerBannerStack.tsx`, `apps/web/src/components/chat/MessagesTimeline.logic.ts`, `apps/web/src/components/chat/MessagesTimeline.test.tsx`, `apps/web/src/components/chat/MessagesTimeline.tsx`, `apps/web/src/components/RightPanelTabs.tsx`, `apps/server/src/provider/Layers/ClaudeProvider.ts`, `apps/desktop/src/backend/DesktopBackendConfiguration.ts`, `apps/web/src/index.css`, `packages/contracts/src/model.ts`, and `packages/shared/src/model.test.ts`; preserve upstream's timeline minimap, live-follow/free-scroll model, sent-message anchoring, composer overlay inset handling, release metadata, middle-click tab close, WSL warmup, Claude model catalog, and ultrathink styling while keeping the fork's workspace-scoped provider skill rendering, subagent child-thread navigation, command/file activity rows, source-control/sidebar glue, terminal availability guards, full-width conversation/composer defaults, VS Code host width preferences, and fold-settling `maintainScrollAtEnd` suppression.

The latest worktree pick pass also brought in timeline anchor/minimap interaction fixes, keyboard and scrollbar cancellation for live-follow scrolling, VS Code host conversation-width application to timeline rows, composer banners, the composer, branch toolbar, and minimap gutter decisions, project-script terminal launch extraction into `runProjectScriptInTerminal(...)`, source-control right-panel surface gating extraction into `ChatView.sourceControl.ts`, and bounded buffering for live thread-detail events emitted during initial snapshot loading. The latest comment-driven pick-backs also keep live-edge jumps from retaining stale timeline anchor restore state, clear pending timeline anchor restore state when resets or send-anchor prep supersede it, keep timeline scroll-listener setup stable across streaming entries and retry it if setup exhausts before the scroll node exists, clear exhausted anchor-positioning attempts so live-follow can recover, clear the active anchor index and return timeline mode to following-end when anchor positioning exhausts, align `ProviderSkillsLister` with the `WorkspacePaths` namespace import convention, wait after opening action terminals before writing commands, treat post-open action-terminal readiness interruptions as non-writable failures, scope project-action terminal reservations by thread, expire fast or missed launch reservations, avoid reserved fallback terminal id collisions, key stash panel expansion/detail caches by stable stash identity, keep the source-control right-panel surface visible when host VCS display preferences hide the panel entry, and refresh cached source-control panel snapshots/details correctly. Earlier review ports remain part of the fork inventory: WSL backend routing/distro-path fixes, host-connected terminal action gating, the dedicated Archive settings panel and its action/review follow-ups, shared changed-file/diff path matching in `diffRendering`, terminal-backed project action candidate classification, project-action terminal launch reservations, source-control thread metadata routing extraction from `ChatView`, draft source-control metadata updates, structured metadata error handling, stale source-control metadata error suppression, hook-lifetime source-control metadata sequencing, status-derived default-branch detection, activity-row preview/keyboard follow-ups, archived-action local-API fallback, archive grouping/search helper extraction and bulk-action hardening, colon-safe archive project grouping keys reused by the Archive panel UI, activity detail patch merging/deleted-file/MCP-row handling, basename-safe changed-file/diff matching, stable same-timestamp unkeyed tool-update ordering, terminal-backed project action readiness/probing hardening, source-control panel review fixes, stale branch worktree path filtering, rename-diff fallback suppression, VCS stream local-watcher retention before initial snapshots and for sibling worktree updates, cwd-scoped source-control working-tree enrichment, selected-file commit/merge argument hardening, interrupted branch-detail load suppression, Codex workspace-skill invalid-cwd normalization, VS Code persistence cleanup coverage, and stricter subagent interrupt turn targeting. These ports intentionally adapt the isolated worktree fixes onto current `main` instead of replaying whole split-branch trees, because several worktrees lack root-owned VS Code, provider-skill, source-control, and chat customizations.
...
- The Effect-based readiness wait exposes a strict typed-error path for attach failures and prompt timeouts, preserving structured attach causes for diagnostics, while the project action command path deliberately keeps the existing best-effort fallback that writes after failure/timeout instead of blocking the action.
...
- Terminal UI controls should be unavailable whenever host terminal support is disabled, no active project exists, or the active project host is disconnected, so hosts such as VS Code do not expose terminal actions that cannot run. Existing open terminal surfaces may remain closable after disconnect, but opening, splitting, and project-action runs require a connected host.
...
The VS Code host bootstrap uses its own `T3HostLocalEnvironmentBootstrap` contract instead of overloading upstream desktop-local backend bootstraps. Preserve the separation: upstream desktop bootstraps use stable backend ids such as `primary` and `wsl:default`, while the VS Code host bootstrap carries the concrete environment id and bearer token. WSL folder routing should keep passing stable desktop backend ids through IPC while resolving distro paths against the concrete running distro.
...
Thread archive/delete lifecycle behavior is enforced server-side in the orchestration decider: archiving or deleting a parent thread cascades through active subagent descendants before the parent event, and force-deleting a project delegates through lifecycle roots so descendant subagents are not double-deleted. The sidebar also treats archive/delete as root-thread lifecycle actions, hiding those row actions for subagent children and failing closed when a selected thread key no longer resolves before multi-select delete.
...
13. Terminal UI controls stay gated on host terminal support, an active project, and a connected project host; existing open terminals remain closable after disconnect.
...
17. Activity row path previews and row-target-only keyboard toggles remain preserved for file-change-style tool rows.
18. Source Control default branch detection honors the status-reported default branch before falling back to `main` or `master`.
