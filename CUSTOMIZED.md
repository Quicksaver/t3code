...
No local behavior was retired by this merge. The previously retired `?diff=1` right-panel override remains absent because upstream owns diff opening through dedicated diff panel state. Upstream added structured failure details for mobile native static checks, macOS passkey signing, desktop user-data probes, relay activity-row persistence, bootstrap startup, preview session keys, mobile pairing payloads, unavailable Bun PTY operations, terminal PTY spawn failures, mobile notification settings, review path resolution, preferred editor preconditions, and relay domain labels. These upstream changes are authoritative; the local VS Code host bootstrap, custom Version Control panel, subagent threading, wide conversation defaults, workspace skill loading, terminal action reuse work, and mobile EAS ownership remain preserved around them.

Concrete merge notes from this merge:

- No textual conflicts occurred. Upstream's structured-error work landed cleanly in `apps/desktop/src/app/DesktopAppIdentity.ts`, `scripts/build-desktop-artifact.ts`, `scripts/mobile-native-static-check.ts`, `apps/server/src/bootstrap.ts`, `apps/server/src/review/ReviewService.ts`, `apps/server/src/terminal/BunPtyAdapter.ts`, `apps/server/src/terminal/PtyAdapter.ts`, `apps/web/src/components/preview/usePreviewSession.ts`, `apps/web/src/editorPreferences.ts`, `apps/mobile/src/features/connection/pairing.ts`, `apps/mobile/src/features/agent-awareness/notificationPermissions.ts`, `infra/relay/src/agentActivity/AgentActivityRows.ts`, and `infra/relay/src/deploymentConfig.ts`.
- The local Mobile EAS project ownership in `apps/mobile/app.config.ts` was not touched; upstream's native static-check and pairing error changes do not alter the `quicksaver` owner or local preview build project id.
- The local terminal-backed project action reuse remains in `apps/server/src/terminal/Manager.ts`, `apps/web/src/projectScriptTerminals.ts`, `apps/web/src/state/projectActionTerminal.ts`, and `apps/web/src/components/ChatView.tsx`; upstream's new PTY adapter error structure is complementary and should be preserved on future terminal conflicts.
- The local conversation activity rendering remains in `apps/web/src/session-logic.ts` and `apps/web/src/components/chat/MessagesTimeline.tsx`; upstream's relay activity-row persistence error structure is server/relay-side and does not replace the custom expandable command and file-change rows.
- The local VS Code host bootstrap and Version Control panel surfaces remain separate customizations; upstream's bootstrap/editor/review error improvements should be kept if future conflicts touch `apps/server/src/bootstrap.ts`, `apps/web/src/editorPreferences.ts`, or `apps/server/src/review/ReviewService.ts`.
...
- `apps/server/src/sourceControl/*` keeps the local remote-repository targeting for GitHub, GitLab, and Azure DevOps change-request discovery while accepting upstream's source-control service refactor and decoder/import naming.
