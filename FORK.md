...
Windows hub processes invoke the SDK avdmanager and sdkmanager Java entry points directly, preserving argument boundaries and avoiding the Unix launcher paths used by the pinned hub. This applies to local and SSH hosts; an SSH hub started without the current adapter is replaced on its next startup. Removal and adapter replacement inspect Windows process command lines through CIM, retaining the owner, entry path, and port checks before stopping a hub. SSH hosts need a non-interactive POSIX-compatible `sh`, including on Windows. Device panel diagnostics wrap long unbroken text within the panel.
...
- Keyboard input retains upstream's native Windows guest routing and composer-focus isolation, with the control-epoch check inside the bounded session callback. Click, type, and wait operations clamp their desktop input timeout to the renderer host's remaining monotonic operation budget. A caller-supplied input timeout can shorten that boundary but cannot outlive the host request; when the input omits a timeout, the remaining request budget is forwarded instead of restarting the desktop default.
...
- Foreground presentation is selection-aware across both the right-panel browser surface and preview mini-player. Browser-specific selectors preserve upstream's floating device streams without mistaking an unrelated device stream for the target browser. A stale renderer `visible` flag cannot keep an unselected guest classified as active and suppress a requested background capture after either selection changes; selected, visible guests still use the normal foreground path.
...
- Recording startup, settings hydration, paint readiness, queued capture grants, first-frame acquisition, capture shutdown, recorder settlement, blob conversion, and artifact persistence all stay inside the remaining operation budget. Stop retries await the original encoder-stop promise before reading chunks. Upload failures preserve their actual cause, and genuine native cleanup errors remain visible after startup timeout. Recording cleanup is manager-scoped. A timed-out stop preserves accumulated chunks and retryable renderer state, shares any in-flight artifact save across retries, and uses a validated idempotency key for deterministic desktop artifact paths so retrying cannot create duplicate files. Recording upload retries receive a fresh bounded deadline after the prior deadline expires, while artifact finalization retains its idempotency key. Failed-start cleanup remains bounded while always releasing its renderer slot, and appearance retries reapply against a replacement guest instead of mutating stale tab state.
...
- Codex root interruption reads the provider history mode, using `thread/read` with `includeTurns: true` for legacy threads or ascending `thread/turns/list` pages for paginated threads. It selects the most recently started `inProgress` turn, with one two-second deadline covering the history-mode read and all pages. When either candidate lacks `startedAt`, provider response order is authoritative and the later entry wins. The cached session turn is read lazily only after the live lookup fails, dies, or times out; unexpected lookup defects are logged before fallback, while a successful lookup with no active turn returns without reviving a stale cached id.
- Root stop preserves upstream's bounded best-effort child interruption fan-out before resolving the root's authoritative live turn. A child fan-out failure must not prevent the root interruption path from settling. The session-stop regression follows upstream's failure semantics: a failed provider stop retains the session state even if failure recording also fails, and a later successful retry terminalizes it.

Conflict guidance:

- `ChatView.localDispatch.ts` owns the dispatch snapshot, exact-message-id acknowledgement, and dispatch hook. Keep message dispatch and new-thread busy variants distinct; preserve submission intent, reconnect guarding, worktree preparation, latest-user-message timing, and the guarded turn/session fallback. `allocateMessageDispatch` allocates and begins a dispatch before returning its `MessageId`; Multi-model fanout and plan implementation use `beginNewThreadBusyState`; fanout preserves worktree preparation and submission intent until its background requests own their threads. Plan follow-up defaults to foreground work.
...
- Shared search ranking, timestamp sorting, and action locks live in `packages/client-runtime/src/state/archivedThreadList.ts`; web and native keep their grouping and bulk orchestration in their respective Archive helpers. Fork integration preserves the updated-time fallback for missing or invalid archive timestamps before falling back to creation time. Archived groups retain the complete project shell, including favicon and environment metadata, and expose collision-safe keys so project ids containing separator characters do not collapse expansion state or React row identity. When a bulk runner throws, it stops scheduling new work, waits for active workers, preserves completed success/failure/skipped counts, surfaces the underlying exception messages, and refreshes the affected environment even after failure.
...
- Native row reservations use JSON tuple identities, while dismissal uses the same scoped thread key registered by live and archived rows. Native row and bulk actions use owning collision-safe reservations acquired before confirmation, and their executor identities serialize the environment/thread tuple instead of joining it with an ambiguous separator. While a target is reserved, overlapping swipe, context-menu, row, and bulk controls remain disabled; visible busy indicators begin only after confirmation succeeds. Rejected duplicates cannot release another action's reservation, canceled confirmations release their own targets, and bulk reservations cover the complete target set before the confirmation dialog opens.
- Native settings share the exact `General`, `Appearance`, `Legacy`, `Archive`, and `App` tail in that order for local and Connect-configured modes. Upstream owns the shared settings list and direct route registration, including the legacy waitlist alias that opens Sign in.
- Upstream `Stack.tsx` registers the customized Archive screen directly in the header-owning settings stack and keeps the legacy `SettingsWaitlist` alias pointed at `SettingsAuth` in the outer authentication stack. Archive environment labels use the shared styled tooltip.
...
Web and mobile archive visibility uses upstream's shared `threadLifecycle.ts` optimistic projection. Archive acknowledgements, interruption, and rollback stay environment-scoped; there is no separate web archive store. Command-palette and sidebar filtering compose that projection with native-subagent visibility. Integrated IndexedDB retains published version 7, including the earlier archive and activity-cache repairs.
...
- The upstream five-minute `ThreadResumeCache` retains the archive generation and eviction listener after the detail subscription closes. Eviction clears its warm body, and generation checks before and after disk reads prevent failed removal or concurrent archive/restore from admitting stale body data or cursors. Body-free deletion tombstones survive shell-first eviction and detail-owner disposal. Atom expiry releases the listener and generation state. Detail-owner eviction and revival recheck ownership inside the shared cache semaphore, and a failed same-owner eviction remains retryable. Batched archive and unarchive events use the individual event path so they retain cache eviction and revival effects. Terminal deletion keeps the fork persistence lock and writes its body-free resume snapshot after eviction.
...
- The missing-thread persistence semaphore survives warm-cache atom owner replacement. Terminal deletion is uninterruptible while waiting for an old finalizer save and then removes the cache through the shared archive/deletion generation boundary. The retained cache still honors data-savings invalidation, body-free tombstones, and idle eviction. Replay batches containing archive or unarchive events use the ordered lifecycle path so cache eviction and restoration are preserved.
...
- Magi shares the core `ProviderService.compactThread` completion barrier. Only an explicit native-compaction receipt for that participant releases the barrier; ordinary turn completion cannot release it. Usage is reread after compaction. Standalone native child shells reset `activeMagiRun`, pull-request metadata, and manual ordering instead of inheriting their parent's state.
- Integrated lineage accepts root, native subagent, and Magi relations. One postorder traversal owns archive, unarchive, and delete across mixed trees. Projection mapping retains both the published generic lineage columns and the standalone Magi compatibility columns, including narrow runtime-context reads. A Magi participant under a native child shares the top lifecycle root's cold-storage restore and hot-row reservation.
...
`rightPanelSurfaceActions.ts` is the canonical descriptor for availability, activation, shortcuts, empty-state and menu order, instance policy, disabled presentation, and badges. Version Control uses `V`, appears first in the empty launcher, and remains last in the compact add menu. Magi participates in the same descriptor with its run badge and shortcut `G`, leaving upstream Device on `M`. `normalizeSourceControlRightPanelPresence` removes only Source Control when unavailable, preserves pull-request and File surfaces, and selects the first remaining visible surface when necessary. Browser runtime-tab resolution, audio, mute-menu, preview mini-player, and Agents reconciliation must leave an existing Source Control surface visible, active, and present exactly once.
...
A normal ahead-branch push snapshots connected peers before pushing. A peer that remains clean with no local commits ahead of the same normalized remote branch is fetched, rechecked, and fast-forwarded afterward; changed, disconnected, differently tracked, or failed peers remain untouched, and peer failures cannot make the completed source push appear failed.
