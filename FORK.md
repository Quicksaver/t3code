...
Generated after the bounded worktree history audit and upstream refresh through `e2d4d12a81516b55abbecdc64794971f781cacd8`. That commit is the integration boundary shared by the refreshed worktree branches; newer upstream commits remain intentionally outside the merge.

For diagnosing existing verification failures, use the host-local linked worktree for branch `base/main`; its current Windows path is `E:\Projects\t3code.worktrees\base-main`. It was created from unmodified `upstream/main` at `230c5d4a5cc31656de3d46719e7f4d2a13369991`, the upstream boundary named by the latest `merge(worktrees)` reconciliation commit `32679a1f317b1879c4deb7765cf11a91858b10db`. Before `$update-worktrees` updates any fork worktree on a host, it fast-forwards that host's control to the fetched `upstream/main`, so both use the same upstream boundary. Obtain native results through the main checkout's `scripts/worktree-baseline.ts ensure`; agents must not install or run checks in the control worktree directly. The helper returns the cached manifest for that exact commit and host/toolchain fingerprint or elects one caller to produce it while concurrent callers wait. A failure seen in a fork worktree is pre-existing only when the same failure appears in the same-host manifest. If it does not, fix it in the fork worktree that introduced or exposed it.
...
- Preview open automation resolves the persisted browser defaults once before it reads or mutates preview session state. One presentation decision drives tab creation, reveal behavior, and readiness: explicit `open` or legacy `show` input wins, while omitted input follows `autoShowFloatingPreview`. The resolved defaults stay pinned across asynchronous settings hydration and reused-tab reconciliation, preventing a late settings epoch from changing the request midway through its lifecycle.
- Browser development now uses the shared single-origin proxy and origin-derived HMR path described above. Desktop development still generates IPv4 HTTP, WebSocket, and web URLs, pins Vite through `HOST=127.0.0.1`, and derives explicit HMR host/protocol/port values from `VITE_DEV_SERVER_URL` when supplied. Server-only backend URLs and the default Vite listener also remain on IPv4 loopback, preventing environment-port navigation from resolving the backend as IPv4 while the related Vite listener is reachable only through IPv6 `localhost`.
- The primary pairing route watches for later URL-fragment changes while it remains mounted. Navigating an already-loaded `/pair` document to `/pair#token=...` claims each new token once, removes the secret fragment, and queues the normal pairing exchange without requiring a reload or a second desktop window. Pairing submissions execute serially and a rejected exchange releases the queue for the next fragment, so rapid fragment navigations cannot overlap token exchanges.
...
- An active-thread send allocates its user-message id before entering local dispatch, and the dispatch API requires that exact id. The send remains locally busy only until the server projects the matching user message. The acknowledgement search checks all projected user messages, not only the newest one, so a later message from another client cannot hide the expected steer. The exact projected message id is authoritative even when the captured session state is stale, ready, or absent. Unrelated projected user messages must not acknowledge the dispatch, and steering the existing running turn must not wait for a new turn or session transition before re-enabling the composer. Implementing a plan in a separate new thread uses its own busy-state variant and is cleared explicitly instead of observing projections from the source thread.
...
Regression coverage lives in `apps/web/src/components/ChatView.logic.test.ts`, `apps/server/src/provider/Layers/CodexInterruptResolution.test.ts`, and `apps/server/src/provider/Layers/CodexCollabRuntime.integration.test.ts`. Keep coverage for consecutive in-turn steers, exact-message acknowledgement across the projected message history, timestamp-based live-turn selection, lazy lookup timeout/failure fallback, successful empty reads that suppress stale interrupts, and root interruption after best-effort child fan-out.
...
- Web archive-row and project-bulk delete confirmations use the shared themed confirmation dialog through `LocalApi`; they do not fall back to browser-native `window.confirm`. The destructive variants retain explicit labels and copy for single-thread, filtered-project, and full-project deletion.
...
- Archive grouping, search ranking, sort state, and project bulk-action concurrency live in `apps/web/src/components/settings/ArchiveSettings.logic.ts` on web and `apps/mobile/src/features/archive/archivedThreadList.ts` on native. Archived groups retain the complete project shell, including favicon and environment metadata, and expose collision-safe keys so project ids containing separator characters do not collapse expansion state or React row identity. Bulk actions preserve completed outcomes and refresh archived threads after every attempt.
...
- Native settings share one contract for the local/configured section tail, Archive route metadata, and the legacy `waitlist` deep-link alias. After the Connect waitlist was retired, that alias opens the Sign in route while preserving old links; both settings modes continue to expose Archive in the same position.
- Native route ownership is explicit: Archive belongs to the content stack that owns its header, while the legacy waitlist alias belongs to the outer authentication stack. Archive environment labels use the shared styled tooltip rather than a native title affordance.
...
- Project removal explicitly opts archived shells into deletion even when no live rows are visible in the sidebar. Grouped project removal derives each member's command options from that member's own live-thread set, so mixed groups force-delete only members with known live threads and use archived-only cleanup for the others. The server still rejects an unseen live thread, and both sidebar variants warn that archived conversation history is included for single projects and grouped projects. Removal emits the same subagent-aware per-root deletion events for archived shells, so already-cold thread trees pass through the durable lifecycle worker and lose their hot shells, archive manifests, and compressed chunks before cleanup completes.
...
- Destructive deletion fails closed before SQL or cold-storage removal when provider stop, terminal close, or provider-log writer shutdown fails. The durable cleanup entry remains retryable, so operational cleanup failures cannot silently delete the remaining conversation state.
- Lifecycle command receipts remain in hot storage while archive or restore work is queued so retries can recover their durable command outcome. A stale restored reservation cannot overwrite a newer archive epoch, missing lifecycle queue rows are reconstructed from manifests and shells during recovery, and an unavailable environment catalog does not cause live thread references to be treated as absent. Process-local restore ownership distinguishes an actively restored root from an orphaned `restored` manifest; startup recovery re-archives orphaned restore reservations discovered after a restart while leaving a restore still owned by the current process alone.
- Unarchive restores `cold`, `restored`, or `cleanup_pending` bundles before dispatching the domain command. Still-hot archived rows receive a `restored` manifest reservation before the archive-tree lock is released, preventing queued lifecycle work from moving them cold between the restore check and the command commit. Each reservation is scoped to its original archive timestamp, so a later archive epoch can replace a stale `restored` manifest and move its hot rows cold even if earlier restore finalization failed. A transient restore failure returns an error without writing a rejected command receipt, allowing the same command id to retry after storage recovers. Partial restore failures roll back restored rows and files, and the restore transaction commit is masked against interruption so committed data cannot be mistaken for a failed restore. A rejected or failed command re-archives restored or reserved rows and files, while a successful command finalizes the cold bundle only when that request actually performed a restore; replaying an already accepted unarchive command retries that finalization so a prior cleanup failure cannot strand restored archive data. This prevents unrelated or already-hot unarchive commands from deleting archive data. If storage cannot restore or reserve the archived conversation, the command is rejected before an active-shell event can commit. Once SQL restoration commits, later publication or metrics failures cannot re-archive the restored data.
- Migration `035_ThreadColdArchive` registers existing archived conversations for background conversion, `036_DeletedThreadCleanupQueue` registers legacy soft-deleted conversations, and `037_SubagentColdArchiveGlue` applies lineage-aware archive grouping after idempotently ensuring the cold-storage base. Migration `038_ProjectionThreadsSettled` preserves the fork's settled-column slot, `039_ProjectionThreadsSnoozed` carries upstream snooze columns, `040_ProjectionThreadTitleRegeneration` carries upstream title-regeneration state, and `041_ThreadColdArchiveCompatibility` reruns cold-archive setup for databases that followed the earlier divergent upstream history. Upstream pinning, turn-keyset, pin-order, project-default-mode, and project-favicon migrations are rehomed idempotently at 42 through 46. Migration `047_ThreadStorageLifecycleCompatibility` reruns both lifecycle bases for databases that already recorded the complete upstream migration sequence through 40. Magi projections, active-conversation uniqueness, and proposal terminology occupy migrations 48 through 50; migration `051_MagiThreadColdArchiveGlue` normalizes existing Magi participant manifests onto their ordinary lifecycle root so cold trees converge with the current archive contract. Upstream auth-session client connections, linked pull-request projection, and un-settle timestamps are registered through fork-compatible wrappers at migrations 52 through 54. Before migration dispatch, the runner recognizes the exact divergent upstream history at 35 through 40 and removes only those markers without deleting their schema or data; the canonical sequence can then replay 35 through 54. Parent relation remains published at 33, and root-id backfill at 34 first ensures those lineage columns so a database recorded only through upstream settlement migration 33 also converges safely.
...
- Optimistic archive filtering applies consistently to project rows, project statuses, project activity sorting, command-palette project and thread actions, keyboard navigation, and prewarming so an archived conversation cannot remain visible or become the latest project target through a secondary list model while its durable lifecycle work finishes.
...
- `t3-sqlite-state` defaults to the hot state database, derives state and archive paths from the shared server configuration, and supports `--database archive` as the read-only inspection path for application-managed cold archive manifests and chunks.
...
- When WebSocket resume falls back to a fresh snapshot, the server returns a dedicated `OrchestrationThreadNotFoundError` carrying the missing thread id and a stable derived message. The client recognizes that typed stream failure and applies the same cache removal and deleted-state transition for a warm cached thread; other snapshot failures retain their transient retry behavior.
...
- `resolveThreadDetailRef` is the canonical gate for draft and server routes. It returns a subscribable reference only when the shell is ready, while explicit wait-for-shell callers and known local drafts remain gated without duplicating route-specific readiness rules.
...
- A resolved or canonical Codex executable inside an npm global installation uses `npm install -g --allow-scripts=@openai/codex @openai/codex@latest`. Standard Windows npm shims under `%APPDATA%\npm` count as npm installations even though they are command files rather than Unix-style symlinks.
- Existing Vite+, Bun, pnpm, and Homebrew installation detections keep their package-manager-specific update commands.
- Every other Codex installation uses `codex update`. This includes the Unix install-script path at `~/.local/bin/codex`, the Windows install-script path under `%LOCALAPPDATA%\Programs\OpenAI\Codex\bin`, unresolved bare commands, and explicit paths that do not identify a supported package manager.
- Provider status snapshots persist the selected command in `<t3-home>/caches/codex.json`, so install-script installations expose `updateCommand: "codex update"` after the provider refreshes.
- Claude, OpenCode, and future package-managed providers retain their existing fallback behavior unless their maintenance definition explicitly supplies its own fallback update.
...
Focused regression coverage lives in `apps/server/src/provider/providerMaintenance.test.ts` and `apps/server/src/provider/Drivers/CodexDriver.test.ts`.
...
Mobile Thread List v2 applies contextual visibility before active/settled partitioning, retains only the exact selected terminal shell without promoting it or restoring hidden terminal ancestors, keeps terminal descendants searchable as navigation-only rows, carries stored depth into rendering, and rebases filtered search rows. Each conversation with running descendants shows a recursive running total, nested generations start collapsed, and expanding one row reveals only eligible running children as compact status rows at stored lineage depth. Root lifecycle actions stay hidden from all subagent rows.
...
Projection bootstrap replays the complete event backlog rather than stopping at the default read-page limit. Thread creation and metadata updates share one parent-relation mapper that preserves root, native-subagent, and Magi lineage fields; keep the explicit SQL read and upsert conflict guards aligned with that mapping.
...
Disabled archive buttons remain the pointer target so clicks cannot fall through to row navigation. Individual button and context-menu archive requests share a per-thread in-flight guard, preventing duplicate confirmations or mutations until the current action settles.
...
Thread source-control metadata update failures should surface on the thread without overwriting unrelated thread errors, and successful source-control updates should clear only the source-control metadata error for that thread. A per-thread metadata queue keeps sequencing monotonic for the hook lifetime so a reopened thread key cannot let an old in-flight failure overwrite a newer successful checkout. Each queued request snapshots every still-unacknowledged predecessor guard when it starts; subscription observations remain authoritative, acknowledge only fresh request targets, and reject lagging predecessors while still allowing a later legitimate return to an earlier branch. Rapid chained transitions therefore retain their guards across completion, failure, and in-flight acknowledgement without replaying stale enqueue state. Server-thread updates carry the active or loading-shell branch as `expectedBranch`, allowing the server to reject a stale metadata write instead of overwriting a newer branch/worktree transition.
...
The fork keeps frequently mounted Git and preview discovery surfaces fresh without continuously repeating their most expensive subprocess work.

Expected behavior:

- Git ref lists use a persisted, generation-aware first-page cache rather than periodic polling. A connected generation performs one live refresh, failures retry with bounded exponential backoff, generation changes cancel stale work, filtered/cursor pages remain live-only, and inactive atoms expire after 30 seconds. The branch selector paginates on demand, while the Diff panel comparison-ref menu explicitly refreshes when opened and resets its retained query without starting replacement polling.
- Ref-affecting mutations invalidate both the shared server snapshot and persisted client ref state after real mutation attempts. Validation-only failures and cache-hit automatic fetches avoid redundant invalidation, while failed, interrupted, or partially applied mutations still refresh consumers without masking the original error.
- Diff-panel ref-menu refreshes and preview port scans are reentrancy-safe: an active request records one queued rerun, drains it after the current work settles, and cannot lose a refresh edge. Closing a query resets its retained state without starting replacement polling.
- Preview port discovery performs one immediate scan when the first subscriber retains it, rather than scanning once for retention and a second time for subscription initialization. Retention carries the configured server URLs into the unified scanner, subscription replays the retained projected snapshot, and candidate readiness results are reused for 15 seconds when the URL and listener identity are unchanged while PID or expiry changes force revalidation. Snapshot publication, listener registration/replay, and listener removal share one notification semaphore so a blocked stale replay cannot arrive after a newer scan broadcast; a listener whose initial replay fails is rolled back before it can block later broadcasts. Subscriber callbacks run from per-listener notification queues outside the global notification lock, and scan serialization is released before awaiting their delivery, so reentrant publication and scan requests enqueue in order without deadlocking on their own callback. Notification dequeue, callback delivery, and publisher acknowledgment form one interruption-masked handoff, so scope closure cannot strand a publisher between callback completion and acknowledgment; cleanup removes the registration and interrupts blocked callback work without waiting behind it. An interrupted active scan re-fails its shared in-flight result with the interruption cause rather than reporting the previous snapshot as a successful refresh. Scans are serialized through publication so an older slow scan cannot overwrite a newer snapshot, while final-retainer invalidation uses the same scan lock so a late in-flight scan cannot restore stale projected state. First retention and final release wake the polling schedule so a newly discovered listener promptly adopts the 10-second active interval and an idle scanner promptly returns to 20 seconds. Managed terminal process-set changes trigger an immediate scan. After a non-empty PID-set change, the first identical follow-up report performs one settle scan so a process that binds its port just after registration is still discovered; idle terminal reports do not consume that pending settle scan, and later identical retained reports are no-ops. The broad `lsof` safety-net scan runs every 20 seconds when no server is known and every 10 seconds while a listener is present, preserving discovery for servers started outside T3-managed terminals without a permanent three-second system-wide process sweep.
- Version Control snapshots are incremental after the authoritative initial load. Normal VCS watcher updates refresh current and sibling working-tree slices while reusing cached branches, remotes, stashes, and comparisons. Mount, focus/visibility, explicit actions, and manual refreshes still request a full snapshot. The server compares repository identity, branch, upstream, ahead/behind, provider, and change-request status before accepting an incremental result and falls back to a full snapshot whenever those fields changed. Per-cwd request revisions prevent a slower older request from replacing a newer cached snapshot.
- Automatic `fetch --all` work is coordinated by Git common directory. Concurrent panel instances share the same in-flight request and successful automatic requests remain fresh for five minutes; failures cool down briefly. Manual Fetch actions invalidate that freshness entry and still perform a real all-remotes fetch, so deduplication does not weaken explicit user intent or omit secondary remotes.
...
- `apps/web/src/components/vcsRefMenuRefresh.test.ts`
...
Focused regression coverage lives in `packages/client-runtime/src/state/vcs.test.ts`, `apps/server/src/preview/PortScanner.test.ts`, and `apps/server/src/sourceControl/SourceControlPanelService.test.ts`.
...
2. Codex subagent threading work, including authoritative child and root provider-control cleanup when both the provider request and diagnostic persistence fail, remains preserved as a local customization unless `main` has an equivalent UI-aware subagent architecture; use `SUBAGENTS.md` as the detailed source of truth.
3. Version Control panel work, including the native mobile screen and its Git-menu entry point, context-keyed web state, subscription-acknowledged metadata queue, request-scoped errors, retryable mobile fetches, process-shared server caches, and structural transport-safe error wrapping, remains preserved as a local customization unless `main` has an equivalent agent-aware version-control experience; use `SOURCE_CONTROL.md` as the detailed source of truth.
4. Version Control idle-power safeguards continue to ignore internal `.git/` watcher churn, batch ignored-path classification into one subprocess per debounce window, defer expensive untracked stats and rename detection to visible-row enrichment, and use a conservative automatic remote Git fetch interval unless upstream ships equivalent low-churn behavior.
5. Expanded command activity rows show differing raw command text inline with the other command details.
6. Command output merging preserves meaningful streamed output across blank fallbacks, whitespace chunks, split chunks, and shorter snapshots.
...
11. Activity row path previews and row-target-only keyboard toggles remain preserved for file-change-style tool rows.
...
13. Mobile pending-task rows stay outside the subagent lineage traversal, while thread rows continue to preserve active-path subagent depth, hidden terminal child visibility, and selected-child shell retention.
...
17. Persisted generation-aware Git ref caching and mutation invalidation, interruption-safe preview listener delivery acknowledgments, incremental Version Control snapshots, and common-directory fetch deduplication retain the Performance Improvements safeguards unless upstream provides equivalent low-churn behavior.
...
19. Projection migrations preserve the fork's published ids 33 through 51, ensure lineage before the id-34 root backfill, normalize only the exact divergent upstream markers before replaying the idempotent canonical sequence, and register the upstream auth-session client connection, linked pull-request, and un-settle timestamp migrations at canonical fork ids 52 through 54.
...
22. Activity payload projection keeps bounded client-consumed command metadata, patch, and collab fields while pruning unrelated provider bulk; command output stays server-side until its row is expanded and the single-activity detail endpoint applies the bounded output projection. Preserve the work-log, lazy-detail, and subagent projection contracts together.
...
**Last origin/main commit SHA:** 32679a1f317b1879c4deb7765cf11a91858b10db
**Last upstream/main commit SHA:** f276e632c5a7952655e2455aef3b86c08164b988
**Last post-merge main...upstream/main count:** 1205 ahead / 9 behind
**Last resolved upstream/main...main diff size:** 712 files changed, 115128 insertions, 11808 deletions
