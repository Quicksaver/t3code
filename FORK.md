...
- Production SQLite and `apps/server/scripts/migrate-dev-db.ts` use the single canonical `apps/server/src/persistence/Migrations.ts` core manifest. The former separate `ConversationDataSavingsMigrations.ts` manifest and `Migrations/ConversationDataSavings/` namespace were removed and must not be restored. Migration `035_ThreadColdArchive` registers existing archived conversations for background conversion, `036_DeletedThreadCleanupQueue` registers legacy soft-deleted conversations, and `037_SubagentColdArchiveGlue` applies lineage-aware archive grouping after idempotently ensuring the cold-storage base. Migration `038_ProjectionThreadsSettled` preserves the fork's settled-column slot, `039_ProjectionThreadsSnoozed` carries upstream snooze columns, `040_ProjectionThreadTitleRegeneration` carries upstream title-regeneration state, and `041_ThreadColdArchiveCompatibility` reruns cold-archive setup for databases that followed the earlier divergent upstream history. Upstream pinning, turn-keyset, pin-order, project-default-mode, and project-favicon migrations are rehomed idempotently at 42 through 46. Migration `047_ThreadStorageLifecycleCompatibility` reruns both lifecycle bases before `051_MagiThreadColdArchiveGlue` normalizes existing Magi participant manifests onto their ordinary lifecycle root. Upstream auth-session client connections, linked pull-request projection, and un-settle timestamps are registered through fork-compatible wrappers at migrations 52 through 54; automatic-model cleanup, project auto-pull, lineage convergence, settlement repair, and project-icon migrations are rehomed at 55 through 59. Core migrations use `effect_sql_migrations`; Magi retains its independent `effect_sql_magi_migrations` ledger at IDs 47 through 50 for projections, active-conversation uniqueness, proposal terminology, and core-history compatibility. Startup removes only the frozen historical Magi names from the core ledger, recognizes the exact divergent upstream history at 35 through 47, and removes those conflicting markers without deleting schema or data so the canonical core sequence can replay through 59 before the Magi manifest runs. The dev-database migration command validates both manifests independently. Parent relation remains published at 33, and root-id backfill at 34 first ensures those lineage columns so a database recorded only through upstream settlement migration 33 also converges safely.
...
- Core projection migrations stay in the canonical ledger described under Conversation Data Savings. Magi keeps its independent `effect_sql_magi_migrations` ledger at IDs 47 through 50. Never restore a shared loader or reuse a published Magi id.
...
19. Core projection migrations preserve published ids 33 through 59, ensure lineage before the id-34 root backfill, and normalize only exact divergent markers before canonical replay. Magi retains a separately validated `effect_sql_magi_migrations` ledger at ids 47 through 50; never merge the loaders or reuse a published Magi id.
...
32. Provider-neutral Magi remains reconciled against `MAGI.md`, including its independent migration ledger, provider subscription/upload/dispatch/compaction contracts, complete projection replay and lineage, root-owned checkpoint refresh, run-history query ownership, shared settings structure, and shared mobile icon.
