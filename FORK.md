...
- `$babysit-worktrees` delegates each branch's pull-request comments through `$babysit`, then invokes `$rebuild-main` when fixes were produced. `$update-prs` delegates pull-request publication through `$piz-pr` without doing branch work in the orchestrator.
