...
`apps/server/src/persistence/Migrations/054_ProjectionThreadParentRelation.ts` applies the complete lineage schema after `053_PullRequestFilesViewed`. It adds every parent, root, provider, title-seed, depth, timing, and status column, backfills existing threads as roots, and creates both relation indexes. Projection reads, shells, detail snapshots, reducers, and shell/detail merges preserve `parentRelation`. Shell-stream refetch/coalescing and WebSocket RPC delivery also retain the complete relation.
...
`apps/server/src/orchestration/Layers/ProviderRuntimeIngestion.ts` requests a generated child title only while the persisted shell title is still the launch seed or `Subagent` and its source is not manual. It captures the current title and title-state version before generation and applies the result through `thread.title.generate.complete`, preserving generated provenance. A manual rename or newer generation that lands while the request is pending wins even when the title text is unchanged. The synthetic shell cache refreshes from the persisted result, so a rejected completion cannot replace the winning title in the cache. Every client surface reuses the persisted title.

`makeSyntheticSubagentThreadShell(...)` in the same module is the single missing-child shell constructor. It inherits parent project, model, runtime, interaction, branch, and worktree context; resets child-owned lifecycle, session, activity, pull-request metadata, and manual order keys; and satisfies `Required<OrchestrationThreadShell>` so additions to the shell contract must be handled at this seam.
...
- Child events can synthesize a missing shell from runtime metadata before the normal parent projection catches up. `resolveThreadRuntimeContext(...)` reads lineage and inherited parent fields through the narrow `ProjectionSnapshotQuery.getThreadRuntimeContext(...)` query and falls back to `syntheticChildShellById` until that child has a projected row. Ordinary runtime events need one context query rather than a full shell read.
...
The web stop control and stop-thread keybinding use the same child ownership rules. A child's persisted running relation enables interruption even when its synthetic session is not running; a terminal relation disables it. The input uses the child's latest running turn id when available and otherwise remains child-scoped without a turn id. The provider reactor resolves the provider route through `rootThreadId` and calls the provider with the explicit child turn.
...
`thread.session.stop` on a synthetic child never assumes a separate child provider binding. It uses the live root route when available, then clears the child projection state. Root session stop terminalizes the projected session only after provider success; its failure path awaits diagnostic recording without terminalizing. Child cleanup counts and logs failed diagnostic appends without blocking child terminalization. Normal root interrupt behavior remains provider-runtime-owned.
...
- Child shells remain available for lifecycle status, lineage, recursive running counts, and root-owned lifecycle behavior. `ThreadRouteView.tsx` gates child detail subscriptions and rendering behind standalone visibility in the shared draft/server route view; a disabled direct child route redirects to its immediate parent. Draft promotion retains the existing chat instance.
...
- `Subagent conversations` lives in Settings → General → Behavior. `ChatView.tsx` passes the persisted title map and route callback to `AgentsPanel` only after opt-in; otherwise the panel remains a non-interactive lifecycle roster. Navigable agent rows use the conversation sidebar's rounded shape, content insets, and title/secondary-text hierarchy. Their hover, focus, text, and border colors use the global theme tokens for the content panel; opting in never changes the panel background. Their three-line organization, status indicators, and lineage indentation stay intact.
...
- `partitionThreadListV2Lineage(...)` in `apps/mobile/src/features/threads/threadListV2.ts` supplies the same lifecycle groups to rendering and `getThreadListV2OrderedSection(...)`. Active order uses each group root; pinned order uses its first pinned member through `sortPinnedThreadsByOrderKey(...)`. Both flatten groups in their original lineage order. `resolveThreadListV2OrderingTargets(...)` maps root controls to the canonical section and representative for menu availability and action dispatch, including groups pinned through a child. Pin decoration remains per row. Settled active roots remain ordering anchors, but the planner rejects any move whose writes include them, matching server eligibility. Selection-only terminal rows do not change canonical movement or pending membership.
- Home and navigation-sidebar move menus, `useThreadListActions.ts`, and `state/thread-order.ts` consume one representative per full section, before search, environment filtering, or disclosure. Move up/down crosses whole lineage groups. Hidden children cannot become independent neighbors, and child changes that preserve the section groups do not cancel pending order while root key updates arrive.
- A root dropped into Active uses `getThreadListV2DropUnpinTargets(...)` to clear every pin in its canonical lineage group, including pins held only by children. `useThreadListActions.ts` checks pinning support before dispatch and stops on the first failed unpin. Independently snoozed subtrees retain their pins even when the moved root is also snoozed; hidden terminal children retain their pins. Root unsettle and unsnooze actions remain root-scoped.
...
`ProviderRuntimeIngestion.test.ts` covers generated child-title provenance and same-text manual-rename/newer-generation races with deferred generation and domain-event completion. `ProviderCommandReactor.test.ts` covers child cleanup after combined provider-control and failure-activity append failures. `ChatView.logic.test.ts` covers child lifecycle eligibility and turn ownership for the stop-thread keybinding.
...
- `apps/server/src/persistence/Migrations/054_ProjectionThreadParentRelation.test.ts`
...
changes, use `test-t3-app` for web and `test-t3-mobile` for mobile when integrated verification is requested.
...
Parent-relation columns remain explicit across SQL reads in `ProjectionSnapshotQuery.ts` and the upsert conflict guard in `ProjectionThreads.ts`. `mapThreadSharedFields(...)` centralizes their snapshot, shell, and detail mapping; `mapThreadParentRelationFields(...)` centralizes event-payload mapping for the projection pipeline.
