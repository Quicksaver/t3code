...
- `ComposerPromptEditor.tsx` uses `planComposerControlledUpdate(...)` to refresh skill labels and descriptions in place without moving a focused selection. Prompt changes, including inline context references, rebuild the editor; context records update their chips through `ComposerContextRecordsContext` without a separate editor-rewrite signal.
...
`ProviderCommandReactor.test.ts` covers child cleanup after combined provider-control and failure-activity append failures. `ChatView.logic.test.ts` covers child lifecycle eligibility and turn ownership for the stop-thread keybinding.
...
- `apps/server/src/persistence/Migrations/052_ProjectionThreadParentRelation.test.ts`
...
- `apps/web/src/composer-editor-controlled-update.test.ts`
