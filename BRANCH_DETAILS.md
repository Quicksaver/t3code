...
The shared client-runtime presentation layer owns tool action categories, count-aware summaries, labels, icons, and viewed-image discovery. The web timeline owns grouping and the generic tool-detail path. Upstream incremental timeline and row projection remain intact. Exact zero durations are handled in command details while other duration formatting uses the shared orchestration timing helper. This branch extends that model with an explicit rich-payload contract for command and file activity. The server preserves bounded structured fields, the web session layer merges lifecycle updates, and command or file-change rows render dedicated detail views inside the standard grouped timeline. Thread synchronization preserves applied command lifecycle records during RPC session replacement and resumes after the latest applied event sequence, leaving the web session layer to merge the completed cumulative output into one displayed activity row.
...
- `apps/web/src/lib/workLogActivity.ts` owns provider-specific command/file payload parsing, viewed-image path extraction, top-level and nested tool-call IDs, bounded patch extraction, changed-file discovery, cumulative output/patch merging, and composition with the shared provider-neutral tool presentation metadata. It retains the branch's structured stdout, stderr, exit-code, duration, and patch extraction instead of reducing those fields to a generic text detail. Its shell-wrapper parsing preserves the original command when boundary quotes do not match. Request classification uses `ProviderRequestKind` from contracts and the shared legacy mapper in `packages/client-runtime/src/pendingRequests.ts`, including `mcp_elicitation_approval` and `permission_approval`; an explicit permission kind takes precedence over the legacy request type.
...
- The web connection database v6 upgrade clears only disposable cached thread-detail snapshots, including v5 live command rows that may have lost their deferred-output marker. Connections and credentials remain intact. Integration must advance beyond database versions already used by other branches. The live-event coalescer retains the subscriber's already-projected payload, preserving its negotiated output representation without projecting it again.
...
- Question-answer history shares `MessagesTimeline.tsx` with rich activity disclosures. Its rows use the upstream `workEntryDisplayLabel` from `MessagesTimeline.logic.ts` and include the answer preview in a stable accessible name, while command and file rows retain their Expand/Collapse prefixes. Expanding an answer shows `QuestionAnswerHistory` without duplicating its content in the generic tool-detail body. `ActivityPayloadProjection.ts` preserves upstream question input alongside bounded command and file data.
...
The branch consumes `packages/client-runtime/src/work-log/presentation.ts` and `packages/client-runtime/src/work-log/commandLabel.ts` as upstream-owned dependencies. The presentation module also accepts structured exit codes for the shared failure and success predicates, preserving rich command outcomes after upstream moved those rules out of the web session layer.
