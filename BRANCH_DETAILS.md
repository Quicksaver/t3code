...
- New-task drafts request workspace skills lazily only after a complete `$skill` reference at whitespace, punctuation, or end-of-input. Local drafts resolve the selected checkout or project root, while future-worktree drafts deliberately have no cwd and use provider snapshots until the target directory exists.
