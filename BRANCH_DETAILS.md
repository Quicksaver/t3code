...
- The web connection database v5 upgrade clears only disposable cached thread-detail snapshots, preventing pre-lazy-output caches from hydrating legacy embedded command output after an upgrade. When integrated after another branch has already claimed v5, this migration must move to the next available database version.
