...
- Web composer and timeline skill lookup share the same provider-instance resolver, including settings-adjusted enabled state, availability, provider locks, continuation groups, and deterministic fallback, so disabled or unavailable persisted selections cannot make the two surfaces query different providers.
