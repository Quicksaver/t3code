...
- `apps/server/src/persistence/Migrations.ts` adds one canonical migration after `base/main`.
  `050_MagiProjections` creates the complete final Magi schema, indexes, and proposal terminology.
...
- `apps/server/src/persistence/Layers/ProjectionThreads.ts`,
  ...
  `activeMagiRun` beside `linkedPullRequest`, `branchPullRequest`, and `unsettledAt`. A conflict resolution must preserve
...
- Mobile new-task drafts use upstream's durable draft identity and project metadata. Magi's
