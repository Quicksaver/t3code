...
export const createLockDirectory = async (
  lockPath: string,
  mkdir: (path: string) => Promise<unknown> = NodeFSP.mkdir,
): Promise<void> => {
  for (let attempt = 0; ; attempt += 1) {
    try {
      ...
      // Windows can report EPERM while another owner finishes deleting the directory.
      if (
...
const withLockMutation = async <A>(
  ...
  while (true) {
    try {
      await createLockDirectory(mutationPath);
      break;
    } catch (error) {
...
export class RuntimeSlotManager {
  ...
  async #withStateLock<A>(action: () => Promise<A>): Promise<A> {
    ...
    while (true) {
      try {
        await createLockDirectory(this.#lockPath);
        try {
