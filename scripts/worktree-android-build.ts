...
const NINJA_DIRTY_PATTERN = /(?:'build\.ninja'|build\.ninja) still dirty after 100 tries\b/iu;
...
const GRADLE_LIBRARY_STAGING_BEGIN = "// T3 Code library CMake staging: begin";
const GRADLE_LIBRARY_STAGING_END = "// T3 Code library CMake staging: end";
...
export const configureAndroidCmakeStaging = async (
  ...
  // Library prefab paths can exceed MAX_PATH before Windows normalizes Ninja's ../ segments.
  const rootGradlePath = NodePath.join(
    canonicalWorktree,
    "apps",
    "mobile",
    "android",
  ...
  const rootSource = await NodeFSP.readFile(rootGradlePath, "utf8");
  const libraryBlock = `${GRADLE_LIBRARY_STAGING_BEGIN}\nsubprojects { libraryProject ->\n    libraryProject.plugins.withId("com.android.library") {\n        libraryProject.android.externalNativeBuild.cmake.buildStagingDirectory = new File(\n            "${gradlePath}/libraries", libraryProject.path.substring(1).replace(':', '/')\n        )\n    }\n}\n${GRADLE_LIBRARY_STAGING_END}`;
  const libraryStart = rootSource.indexOf(GRADLE_LIBRARY_STAGING_BEGIN);
  const libraryEnd = rootSource.indexOf(GRADLE_LIBRARY_STAGING_END);
  const rootRewritten =
    libraryStart !== -1 && libraryEnd > libraryStart
      ? `${rootSource.slice(0, libraryStart)}${libraryBlock}${rootSource.slice(libraryEnd + GRADLE_LIBRARY_STAGING_END.length)}`
      : `${rootSource.trimEnd()}\n\n${libraryBlock}\n`;
  await NodeFSP.writeFile(rootGradlePath, rootRewritten);
...
const createOperations = (worktree: string, device: string): AndroidBuildOperations => {
  ...
  return {
    install: async () =>
      runChecked("Workspace install", install.command, install.args, {
        cwd: worktree,
        env: ANDROID_INSTALL_ENV,
      }),
    prebuild: async () =>
      runChecked(
        "Expo clean prebuild",
