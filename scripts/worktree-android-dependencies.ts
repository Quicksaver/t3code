...
export const resolveAndroidNativePackageFromMobile = async (
  ...
  const packageJsonPath = NodeModule.findPackageJSON(
    packageName,
    NodeURL.pathToFileURL(NodePath.join(worktree, "apps", "mobile", "package.json")),
  );
  if (packageJsonPath === undefined) {
    throw new Error(`Cannot find ${packageName} from apps/mobile in ${worktree}.`);
  }
