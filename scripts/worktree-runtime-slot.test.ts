...
describe("worktree-runtime-slot", () => {
  it.runIf(NodeProcess.platform === "win32")(
    "retries a transient Windows directory deletion conflict",
    async () => {
      const mkdir = vi
        .fn()
        .mockRejectedValueOnce(Object.assign(new Error("deletion pending"), { code: "EPERM" }))
        .mockResolvedValue(undefined);
      await expect(createLockDirectory("owned.lock", mkdir)).resolves.toBeUndefined();
      expect(mkdir).toHaveBeenCalledTimes(2);
    },
  );

  it.runIf(NodeProcess.platform === "win32")(
    "does not hide persistent Windows directory permission failures",
    async () => {
      const denied = Object.assign(new Error("denied"), { code: "EPERM" });
      const mkdir = vi.fn().mockRejectedValue(denied);
      await expect(createLockDirectory("owned.lock", mkdir)).rejects.toBe(denied);
      expect(mkdir).toHaveBeenCalledTimes(5);
    },
  );

  it("preserves lock contention and other directory failures", async () => {
    for (const code of ["EEXIST", "EACCES"]) {
      const error = Object.assign(new Error(code), { code });
      const mkdir = vi.fn().mockRejectedValue(error);
      await expect(createLockDirectory("owned.lock", mkdir)).rejects.toBe(error);
