...
function inspectPosixProcessTree(input: {
  ...
  const directChildCommand =
    normalizeChildCommandName(commandByPid.get(input.childPid) ?? "", input.platform) ??
    input.childCommand;
  if (!isInteractiveShellCommand(directChildCommand)) {
    return {
      hasRunningSubprocess: true,
      childCommand: directChildCommand ? truncateTerminalWireLabel(directChildCommand) : null,
      processIds: [...processIds],
    };
  }

  const descendants = [...processIds].filter(
    (pid) => pid !== input.terminalPid && pid !== input.childPid,
  );
  for (const pid of descendants) {
    const command = normalizeChildCommandName(commandByPid.get(pid) ?? "", input.platform);
    if (command && !isInteractiveShellCommand(command)) {
      return {
        hasRunningSubprocess: true,
        childCommand: truncateTerminalWireLabel(command),
        processIds: [...processIds],
      };
    }
  }
...
> {
  ...
  const processIds = new Set<number>([terminalPid, childPid]);
  return {
    hasRunningSubprocess: true,
    childCommand: normalized ? truncateTerminalWireLabel(normalized) : null,
    processIds: [...processIds],
