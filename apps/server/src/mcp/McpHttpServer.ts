...
// Codex MCP tools/call metadata identifies the native caller when children inherit the bearer.
const MagiCallerRequest = Schema.Struct({
  method: Schema.Literal("tools/call"),
  params: Schema.Struct({
    name: Schema.String,
    _meta: Schema.optional(Schema.Struct({ threadId: Schema.optional(Schema.Unknown) })),
  }),
});
const decodeMagiCallerRequest = Schema.decodeUnknownOption(MagiCallerRequest);
      ...
      const call =
        request.method === "POST"
          ? yield* request.json.pipe(
              Effect.map(decodeMagiCallerRequest),
              Effect.orElseSucceed(() => Option.none()),
            )
          : Option.none();
      const nativeThreadId =
        Option.isSome(call) && call.value.params.name.startsWith("magi_")
          ? call.value.params._meta?.threadId
          : undefined;
...
export const getMcpToolInputSchema = (tool: Tool.Any) => {
  ...
  // Effect represents an empty Struct as any non-null value. MCP requires
  // object parameters even for tools with no arguments.
  if (
    Object.keys(inputSchema).length === 1 &&
    "not" in inputSchema &&
    typeof inputSchema.not === "object" &&
    inputSchema.not !== null &&
    "type" in inputSchema.not &&
    inputSchema.not.type === "null"
  ) {
...
const MagiToolkitRegistrationLive = Layer.effectDiscard(registerMagiToolkit()).pipe(
...
const ContextArtifactToolkitRegistrationLive = Layer.effectDiscard(
...
const CombinedToolkitServerLive = isolateMcpServerLayer(
  Layer.mergeAll(
    ...
    MagiToolkitRegistrationLive,
    PullRequestsToolkitRegistrationLive,
    DeviceToolkitRegistrationLive,
  ).pipe(Layer.provideMerge(makeMcpTransportLive("/mcp/all"))),
);
...
const PreviewOnlyServerLive = isolateMcpServerLayer(
  Layer.mergeAll(
    ...
    PullRequestsToolkitRegistrationLive,
    DeviceToolkitRegistrationLive,
  ).pipe(Layer.provideMerge(makeMcpTransportLive("/mcp"))),
);
...
const MagiOnlyServerLive = isolateMcpServerLayer(
  Layer.mergeAll(
    ...
    PullRequestsToolkitRegistrationLive,
    DeviceToolkitRegistrationLive,
  ).pipe(Layer.provideMerge(makeMcpTransportLive("/mcp/magi"))),
);
