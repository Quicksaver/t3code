...
const withControl = <A, E>(
  ...
  Effect.gen(function* () {
    ...
    const caller = yield* broker.resolveInvocation(scope);
    return yield* effect(broker, caller);
...
export const MagiToolkitHandlersLive = MagiToolkit.toLayer({
  magi_control_run: (input) =>
    withControl("magi_control_run", (broker, scope) => broker.controlRun(scope, input)),
