...
const MagiListContextActivitiesTool = readonlyTool(
...
const MagiRecordActionsTool = Tool.make("magi_record_actions", {
  description:
...
const MagiControlRunTool = Tool.make("magi_control_run", {
  description:
    "Pause, resume, or stop a Magi run owned by this conversation, including from a later ordinary turn. Manage an existing run whenever appropriate without asking the user to click panel controls. Pause preserves the run at the next serialized boundary. Resume restores its saved protocol state and returns the required next tool; follow that transition. Stop interrupts active participants and permanently ends the run without undoing actions. Pending approvals and input still use the normal conversation flow. Record uncertain issued action outcomes with magi_record_actions; never replay them automatically.",
  parameters: MagiControlRunInput,
  success: MagiRecoverRunContextResult,
  failure,
  dependencies,
})
  .annotate(Tool.Title, "Control Magi run")
  .annotate(Tool.Destructive, false)
  .annotate(Tool.Idempotent, true);
