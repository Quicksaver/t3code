...
export interface MagiControlHandlers {
  ...
  ) => Effect.Effect<McpInvocationScope, MagiValidationError>;
...
export const proxy: MagiControlHandlers = {
  resolveInvocation: (scope) => activeHandlers?.resolveInvocation(scope) ?? unavailable(),
  ...
  controlRun: (scope, input) => activeHandlers?.controlRun(scope, input) ?? unavailable(),
  listRuns: (input) =>
