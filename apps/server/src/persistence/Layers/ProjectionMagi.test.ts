...
layer("ProjectionMagiRepository", (it) => {
  it.effect(
    "paginates running and finished sibling runs together while retaining their owners",
    () =>
      Effect.gen(function* () {
        const repository = yield* ProjectionMagiRepository;
        const sql = yield* SqlClient.SqlClient;
        const owners = [ThreadId.make("history-alpha"), ThreadId.make("history-beta")];
        for (const [index, owner] of [...owners, ThreadId.make("history-foreign")].entries()) {
          yield* sql`
          INSERT INTO projection_threads (
            thread_id, project_id, title, model_selection_json, created_at, updated_at
          ) VALUES (
            ${owner}, 'history-project', 'History owner', NULL,
            '2026-08-22T03:00:00.000Z', '2026-08-22T03:00:00.000Z'
          )
        `;
          yield* repository.putRun({
            ...persistedRun,
            initiatingReferenceId: `history-reference-${index}`,
            protocol: { ...(persistedRun.protocol as Record<string, unknown>), members: [] },
            detail: {
              ...persistedRun.detail,
              summary: {
                ...persistedRun.detail.summary,
                rootThreadId: owner,
                runId: MagiRunId.make(`history-run-${index}`),
                state: index === 0 ? "deliberating" : "turn-limit-reached",
                startedAt: `2026-08-22T03:00:0${index}.000Z`,
              },
              participants: [],
            },
          });
        ...
        const input = {
          rootThreadId: ThreadId.make("history-parent"),
          ownerThreadIds: owners,
          limit: 1,
        };
        const first = yield* repository.listRuns(input);
        assert.deepStrictEqual(
          first.runs.map((run) => [run.rootThreadId, run.state]),
          [[owners[1], "turn-limit-reached"]],
        );
        assert.isNotNull(first.nextCursor);
        const second = yield* repository.listRuns({ ...input, cursor: first.nextCursor! });
        assert.deepStrictEqual(
          second.runs.map((run) => [run.rootThreadId, run.state]),
          [[owners[0], "deliberating"]],
        );
        assert.isNull(second.nextCursor);
        assert.deepStrictEqual(
          (yield* repository.listRuns({ rootThreadId: owners[0]!, limit: 10 })).runs.map(
            (run) => run.rootThreadId,
          ),
          [owners[0]],
        );
        assert.isTrue(Option.isNone(yield* repository.findActiveRun(input.rootThreadId)));
        for (const owner of [...owners, ThreadId.make("history-foreign")]) {
