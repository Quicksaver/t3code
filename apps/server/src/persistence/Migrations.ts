...
import Migration0066 from "./Migrations/066_PullRequestFilesViewed.ts";
...
export const migrationEntries = [
  ...
  [66, "PullRequestFilesViewed", Migration0066],
] as const;
...
const normalizeDivergentMigrationHistory = Effect.fn("normalizeDivergentMigrationHistory")(
  function* () {
    ...
    // the canonical fork sequence can replay the tail without deleting
    ...
    yield* sql`
      ...
      WHERE
        ...
        (migration_id = 49 AND name = 'ProjectionThreadsActiveOrderKey') OR
        (migration_id = 50 AND name = 'ProjectionThreadPullRequests') OR
        (migration_id = 51 AND name = 'ProjectionThreadMessageContext') OR
        (migration_id = 52 AND name = 'ProjectionThreadTitleState') OR
        (migration_id = 53 AND name = 'PullRequestFilesViewed')
    `;
  },
);
