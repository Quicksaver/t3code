...
export default Effect.gen(function* () {
  ...
  yield* sql`
    ...
    ADD COLUMN parent_activity_sequence INTEGER NOT NULL DEFAULT 0
  `;
  ...
  yield* sql`
    CREATE INDEX idx_projection_threads_parent_relation
    ON projection_threads(parent_thread_id, subagent_status, subagent_started_at, thread_id)
  `;

  yield* sql`
    CREATE INDEX idx_projection_threads_root_relation
