...
layer("033 projection thread parent relation", (it) => {
  it.effect("adds the final lineage schema and backfills existing roots", () =>
    Effect.gen(function* () {
      ...
      const executed = yield* runMigrations({ toMigrationInclusive: 33 });
      assert.deepStrictEqual(executed, [[33, "ProjectionThreadParentRelation"]]);
