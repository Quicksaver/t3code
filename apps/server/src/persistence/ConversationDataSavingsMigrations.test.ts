...
import { runMigrations } from "./ConversationDataSavingsMigrations.ts";
import * as NodeSqliteClient from "@t3tools/shared/nodeSqliteClient";
import UpstreamMigration0035 from "./Migrations/035_ProjectionThreadTitleRegeneration.ts";
import UpstreamMigration0036 from "./Migrations/036_ProjectionThreadsPinned.ts";
import UpstreamMigration0037 from "./Migrations/037_ProjectionTurnsKeysetIndex.ts";
import UpstreamMigration0038 from "./Migrations/038_ProjectionThreadsPinOrderKey.ts";
import UpstreamMigration0039 from "./Migrations/039_ProjectionProjectsDefaultThreadEnvMode.ts";
import UpstreamMigration0040 from "./Migrations/040_ProjectionProjectFaviconPath.ts";
...
currentUpstreamHistoryLayer("044 current-upstream compatibility", (it) => {
  it.effect("creates lifecycle tables after upstream migrations through 43", () =>
    Effect.gen(function* () {
      const sql = yield* SqlClient.SqlClient;
      yield* runMigrations({ toMigrationInclusive: 34 });
      ...
      yield* UpstreamMigration0041;
      yield* UpstreamMigration0042;
      yield* UpstreamMigration0043;
      yield* UpstreamMigration0044;
      yield* UpstreamMigration0045;
      yield* UpstreamMigration0046;
      yield* UpstreamMigration0047;
      yield* sql`
        INSERT INTO effect_sql_migrations (migration_id, name)
        VALUES
      ...
      assert.deepStrictEqual(executed, [
        [48, "ThreadStorageLifecycleCompatibility"],
