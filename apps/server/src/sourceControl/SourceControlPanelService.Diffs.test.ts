...
describe("SourceControlPanelService", () => {
  ...
  it.effect("matches working-tree statistics to paths when opened below the repository root", () =>
    Effect.gen(function* () {
      const service = yield* SourceControlPanelService;
      const snapshot = yield* service.snapshot({ cwd: "/repo/apps/server" });
      const unstaged = snapshot.changeGroups.find((group) => group.kind === "unstaged")?.files;
      const staged = snapshot.changeGroups.find((group) => group.kind === "staged")?.files;
      assert.deepStrictEqual(unstaged, [
        ...
        { path: "inside.ts", originalPath: null, status: "modified", insertions: 2, deletions: 1 },
      ]);
      assert.deepStrictEqual(staged, [
        {
          path: "../web/renamed.ts",
          originalPath: "../web/original.ts",
          status: "renamed",
          insertions: 4,
          deletions: 2,
        },
              ...
              switch (input.operation) {
                ...
                case "vcs.panel.statusPorcelain":
                  return success(
                    [
                      "# branch.head main",
                      "1 .M N... 100644 100644 100644 abc abc ../web/changed.ts",
                      "1 .M N... 100644 100644 100644 abc abc inside.ts",
                      "2 R. N... 100644 100644 100644 abc abc R080 ../web/renamed.ts\t../web/original.ts",
                    ].join("\n"),
                  );
                case "vcs.panel.unstagedNumstat":
                  return success("8\t3\tapps/web/changed.ts\0" + "2\t1\tapps/server/inside.ts\0");
                case "vcs.panel.stagedNumstat":
                  return success("4\t2\t\0apps/web/original.ts\0apps/web/renamed.ts\0");
                case "vcs.panel.stagedNameStatus":
                  return success("R080\0apps/web/original.ts\0apps/web/renamed.ts\0");
                default:
                  return success();
              }
            }),
  ...
  it.effect("keeps enrichment rename paths and deletion statistics relative to a nested cwd", () =>
    Effect.gen(function* () {
      const service = yield* SourceControlPanelService;
      const result = yield* service.enrichWorkingTreeFiles({
        cwd: "/repo/apps/server",
        paths: ["../web/new.ts", "../web/old.ts", "../web/deleted.ts"],
      });
      assert.deepStrictEqual(result, {
        files: [
          ...
          {
            path: "../web/new.ts",
            originalPath: "../web/old.ts",
            status: "renamed",
            insertions: 2,
            deletions: 1,
          },
        ],
        hiddenPaths: ["../web/old.ts"],
      });
    }).pipe(
      Effect.provide(
        makeTestLayer((input) =>
          Effect.sync(() => {
            switch (input.operation) {
              ...
              case "vcs.panel.enrichWorkingTreeFiles.statusPorcelain":
                return success(
                  [
                    "1 .D N... 100644 100644 000000 abc abc ../web/deleted.ts",
                    "1 .D N... 100644 100644 000000 abc abc ../web/old.ts",
                    "? ../web/new.ts",
                  ].join("\n"),
                );
              case "vcs.panel.enrichWorkingTreeFiles.unstagedNumstat":
                return success("0\t7\tapps/web/deleted.ts\0" + "0\t9\tapps/web/old.ts\0");
              case "vcs.panel.enrichWorkingTreeFiles.untrackedNumstat":
                return success("10\t0\t../web/new.ts\0");
              case "vcs.panel.unstagedNameStatusWithUntracked":
                return success("R080\0apps/web/old.ts\0apps/web/new.ts\0");
              case "vcs.panel.unstagedNumstatWithUntracked":
                return success("2\t1\t\0apps/web/old.ts\0apps/web/new.ts\0");
              default:
                return success();
            }
          }),
