...
describe("SourceControlPanelService", () => {
  for (const registeredSibling of [true, false]) {
    it.effect(
      `resolves Git-form project paths for ${registeredSibling ? "registered" : "inherited"} sibling messages`,
      () =>
        Effect.gen(function* () {
          const path = yield* Path.Path;
          const requests: CommitMessageGenerationInput[] = [];
          const projectId = ProjectId.make("windows-project");
          const project = OrchestrationProject.make({
            id: projectId,
