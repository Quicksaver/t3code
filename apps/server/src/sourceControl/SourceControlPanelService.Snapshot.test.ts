...
describe("SourceControlPanelService", () => {
  ...
  it.effect("surfaces open pull request base branches only when the local branch is behind", () => {
    const listChangeRequestInputs: Array<{
    ...
    return Effect.gen(function* () {
      ...
      assert.deepStrictEqual(listChangeRequestInputs, [{ limit: 100 }]);
    }).pipe(
      Effect.provide(
        makeTestLayer(
          (input) =>
          ...
          },
          {
            github: SourceControlProvider.SourceControlProvider.of({
              ...
              listChangeRequests: (input) => {
                listChangeRequestInputs.push({
                  ...(input.headSelector ? { headSelector: input.headSelector } : {}),
                  ...(input.limit !== undefined ? { limit: input.limit } : {}),
                });
                return Effect.succeed<readonly ChangeRequest[]>([
                  {
                    provider: "github",
                    number: 42,
                    title: "Feature",
