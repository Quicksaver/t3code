...
describe("SourceControlPanelService", () => {
  ...
  it.effect("refreshes only working-tree slices when repository status is unchanged", () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.isTrue(includePullRequestOptions.length > 0);
      assert.isTrue(includePullRequestOptions.every((include) => include === false));
    }).pipe(
      Effect.provide(
        makeTestLayer(
          (input) =>
            ...
            }),
          {
            status: (_input, options) =>
              Effect.sync(() => {
                includePullRequestOptions.push(options?.includePullRequest);
                return {
                  ...localStatus,
                  refName: "main",
                  isDefaultRef: true,
