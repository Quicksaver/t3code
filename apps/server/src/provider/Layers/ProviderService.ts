  ...
  const sendTurn: ProviderServiceMethod<"sendTurn"> = Effect.fn("sendTurn")(function* (rawInput) {
    ...
    return yield* Effect.gen(function* () {
      ...
      const turn = yield* Effect.gen(function* () {
        const startedTurn = yield* routed.adapter.sendTurn(input);
        yield* directory.upsert({
          threadId: input.threadId,
    ...
    function* (threadId) {
      ...
      if (!routed.adapter.compactSession) {
        return yield* new ProviderAdapterRequestError({
          ...
          detail: "This provider does not expose native session compaction.",
        });
      }
      yield* routed.adapter.compactSession(threadId);
    },
  );
