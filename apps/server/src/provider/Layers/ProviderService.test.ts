...
describe("agent browser access", () => {
  ...
  const startSessionWith = (
    ...
    threadId: ThreadId,
    control?: {
      readonly executionProfile?: "magi-read-only";
      readonly magiControlEnabled?: boolean;
    },
      ...
      const providerLayer = makeProviderServiceLive({
        issueMcpCredential: (request) =>
          Effect.sync(() => {
            ...
            onIssue?.(request.capabilities ?? new Set());
            return undefined;
          }),
      }).pipe(
        Layer.provide(providerAdapterLayer),
  ...
  it.effect("keeps the Magi control credential when agent browser access is off", () =>
    Effect.gen(function* () {
  ...
  it.effect("withholds preview access when the project disables browser access", () =>
    Effect.gen(function* () {
      ...
      const issued = yield* startSessionWith(
        ...
        threadId,
        undefined,
        (entry) => capabilities.push(entry),
        false,
      );
      assert.deepEqual(issued, [threadId]);
      assert.deepEqual([...capabilities[0]!], ["magi-control"]);
    }).pipe(Effect.provide(NodeServices.layer)),
  );
  ...
  it.effect("requests an MCP credential when the project overrides browser access to on", () =>
    Effect.gen(function* () {
      ...
      const issued = yield* startSessionWith(false, threadId, undefined, undefined, true);
      assert.deepEqual(issued, [threadId]);
    }).pipe(Effect.provide(NodeServices.layer)),
  );
