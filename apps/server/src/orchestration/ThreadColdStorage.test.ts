...
layer("ThreadColdStorage", (it) => {
  it.effect("restores cold children and reserves hot descendants with their root", () =>
    Effect.gen(function* () {
      ...
      yield* sql`UPDATE projection_threads SET parent_kind = 'magi',
        parent_thread_id = ${child}, root_thread_id = ${child} WHERE thread_id = ${magiChild}`;
  ...
  it.effect("retains archived PR relations until permanent deletion", () =>
    Effect.gen(function* () {
      const sql = yield* SqlClient.SqlClient;
      const storage = yield* ThreadColdStorage.ThreadColdStorage;
      const threadId = ThreadId.make("thread-pr-relations");
      yield* insertArchivedThread(threadId, "PR-linked thread");
      yield* sql`
        INSERT INTO projection_thread_pull_requests (
          thread_id, host, repository, number, url, source, linked_at
        ) VALUES (
          ${threadId}, 'github.com', 'example/repo', 12,
          'https://github.com/example/repo/pull/12', 'manual', '2026-07-01T00:00:00.000Z'
        )
      `;
      const links = sql`SELECT number FROM projection_thread_pull_requests WHERE thread_id = ${threadId}`;
      yield* storage.archiveThread(threadId);
      assert.deepStrictEqual(yield* links, [{ number: 12 }]);
      assert.isTrue(yield* storage.restoreTree(threadId));
      assert.deepStrictEqual(yield* links, [{ number: 12 }]);
      yield* storage.finishRestoreTree(threadId);
      yield* storage.deleteThread(threadId);
      assert.deepStrictEqual(yield* links, []);
    }),
  );
