...
const noOp = ThreadColdStorage.of({
...
class ArchiveTableValidationError extends Schema.TaggedError<ArchiveTableValidationError>()(
...
class ArchiveChunkKindValidationError extends Schema.TaggedError<ArchiveChunkKindValidationError>()(
...
const THREAD_TABLES = [
  ...
  // Archived PR relations remain queryable alongside their thread shells.
  ["projection_thread_pull_requests", "thread_id"] as const,
...
const make = Effect.gen(function* () {
  ...
  const restoreTreeImpl = Effect.fn("restoreArchiveTreeImpl")(function* (threadId: ThreadId) {
    ...
    if (ownsRestore) activeRestoreRoots.add(String(rootThreadId));
    return ownsRestore;
  });
