...
function makeSyntheticSubagentThreadShell(input: {
  readonly parentThread: Pick<
    OrchestrationThreadShell,
    "projectId" | "modelSelection" | "runtimeMode" | "interactionMode" | "branch" | "worktreePath"
  >;
  readonly childThreadId: ThreadId;
  readonly parentRelation: SubagentThreadParentRelation;
  readonly createdAt: OrchestrationThreadShell["createdAt"];
}): OrchestrationThreadShell {
  const shell = {
  ...
  return children.flatMap((entry): RuntimeSubagentChild[] => {
    ...
    const rawPrompt =
      typeof record?.rawPrompt === "string" && record.rawPrompt.trim().length > 0
        ...
        : null;
    const startsChildTurn =
      typeof record?.startsChildTurn === "boolean" ? record.startsChildTurn : undefined;
    return [
      {
        childThreadId,
        providerThreadId,
...
const make = Effect.gen(function* () {
  ...
  const resolveThreadRuntimeContext = Effect.fn("resolveThreadRuntimeContext")(function* (
    ...
    return projected ?? (yield* Ref.get(syntheticChildShellById)).get(threadId);
  });
  ...
  const maybeGenerateSubagentThreadTitle = Effect.fn("maybeGenerateSubagentThreadTitle")(
    function* (input: {
      ...
      yield* Effect.gen(function* () {
        const child = yield* resolveThreadShell(input.childThreadId);
        if (
          !child ||
          child.titleState?.source === "manual" ||
          (!canReplaceThreadTitle(child.title, titleSeed) && child.title.trim() !== "Subagent")
        ) {
          return;
        }
        ...
        const projected = yield* projectionSnapshotQuery.getThreadShellById(input.childThreadId);
        if (Option.isSome(projected)) {
          yield* Ref.update(syntheticChildShellById, (current) => {
            if (!current.has(input.childThreadId)) return current;
            const next = new Map(current);
            next.set(input.childThreadId, projected.value);
            return next;
          });
        }
      }).pipe(
  ...
  const processRuntimeEvent = (event: ProviderRuntimeEvent) =>
    Effect.gen(function* () {
      ...
      if (!thread) {
        ...
        const syntheticThread = makeSyntheticSubagentThreadShell({
          parentThread,
          childThreadId: parentCollab.childThreadId,
          parentRelation,
          createdAt: event.createdAt,
        ...
        yield* Ref.update(syntheticChildShellById, (current) => {
          const next = new Map(current);
          next.set(parentCollab.childThreadId, syntheticThread);
          return next;
        });
        yield* orchestrationEngine.dispatch({
          type: "thread.create",
            ...
            Effect.gen(function* () {
              ...
              if (!existingChild) {
                yield* Ref.update(syntheticChildShellById, (current) => {
                  ...
                  next.set(
                    child.childThreadId,
                    makeSyntheticSubagentThreadShell({
                      parentThread: thread,
