...
describe("ProviderRuntimeIngestion", () => {
  ...
  it("generates a concise title for projected subagent child threads", async () => {
    ...
    expect(childThread.titleState).toMatchObject({
      source: "generated",
      version: "provider:subagent-thread-title:subagent-title-test",
  ...
  for (const winner of ["manual rename", "newer generation"] as const) {
    effectIt.effect(`preserves a ${winner} with unchanged text during child title generation`, () =>
      Effect.gen(function* () {
            ...
            textGeneration: {
              generateThreadTitle: () =>
                Deferred.succeed(generationStarted, undefined).pipe(
                  ...
                  Effect.as({ title: "Stale generated title" }),
                ),
            },
          }),
        );
        yield* Effect.addFinalizer(() => Deferred.succeed(releaseGeneration, undefined));
        const childThreadId = asThreadId("subagent-title-race");
        const generationCommandId = CommandId.make(
          `provider:subagent-thread-title:${childThreadId}`,
        );
        const completed = yield* harness.engine.streamDomainEvents.pipe(
          Stream.filter((event) => event.commandId === generationCommandId),
          Stream.runHead,
          Effect.forkScoped({ startImmediately: true }),
        );
        yield* Effect.promise(() =>
          harness.emitAndDrain([
            {
              type: "item.started",
              eventId: asEventId("evt-subagent-title-race"),
              provider: ProviderDriverKind.make("codex"),
              createdAt: "2026-01-01T00:00:00.000Z",
              threadId: asThreadId("thread-1"),
              turnId: asTurnId("parent-turn"),
              itemId: asItemId("parent-title-race"),
              payload: {
                itemType: "collab_agent_tool_call",
                status: "in_progress",
                data: {
                  subagentChildren: [
                    {
                      providerThreadId: "provider-child-title-race",
                      childThreadId,
                      parentItemId: "parent-title-race",
                      titleSeed: "Inspect the race",
                    },
                  ],
                },
              },
        ...
        const child = before.threads.find((thread) => thread.id === childThreadId)!;
        const winningCommandId = CommandId.make("winning-title-change");
        yield* Effect.promise(() =>
          harness.dispatch(
            winner === "manual rename"
              ? {
              ...
              : {
                  ...
                  expectedVersion: child.titleState?.version ?? null,
                  needsRefinement: false,
                },
          ),
        );
        ...
        expect(after.threads.find((thread) => thread.id === childThreadId)).toMatchObject({
          title: child.title,
          titleState: {
            source: winner === "manual rename" ? "manual" : "generated",
            version: winningCommandId,
            needsRefinement: false,
          },
        });
      }),
  ...
  it("reopens a completed subagent only when the parent activity starts a child turn", async () => {
    const harness = await createHarness({
      textGeneration: {
    ...
    harness.emit({
      type: "item.completed",
      eventId: asEventId("evt-subagent-resume-interaction-completed"),
      provider: ProviderDriverKind.make("codex"),
      createdAt: "2026-01-01T00:00:30.000Z",
      threadId: asThreadId("thread-1"),
      turnId: asTurnId("turn-parent-interaction"),
      itemId: asItemId("parent-item-interaction"),
      payload: {
        itemType: "collab_agent_tool_call",
        status: "completed",
        title: "Subagent",
        detail: "Message completed child",
        data: {
          subagentChildren: [
            {
              providerThreadId: "provider-child-resume-test",
              childThreadId,
              parentItemId: "parent-item-interaction",
              titleSeed: "Message completed child",
    ...
    await waitForThread(
      ...
      (entry) =>
        entry.parentRelation?.kind === "subagent" &&
        entry.parentRelation.status === "completed" &&
        entry.parentRelation.completedAt === "2026-01-01T00:00:02.000Z" &&
        entry.parentRelation.parentItemId === "parent-item-interaction",
      2000,
      childThreadId,
    );
