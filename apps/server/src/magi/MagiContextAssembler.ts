...
const activitySummary = (activity: OrchestrationThreadActivity): string => {
  ...
  const detail =
    typeof payload === "object" && payload !== null && "detail" in payload
      ...
      : undefined;
  const summary =
    typeof detail === "string" && detail.trim() && detail !== activity.summary
      ? `${activity.summary}: ${detail.trim()}`
      : activity.summary;
  return summary.slice(0, 2_000).trim();
};
