...
const MAGI_PARTICIPANT_TURN_TIMEOUT = Duration.minutes(30);
...
${buildMagiArbitratorPreTurnInstructions(settings.arbitratorPrompt)}`;
  ...
  const resolveConversationContext = (threadId: ThreadId) =>
    Effect.gen(function* () {
      ...
      const project = shell.projects.find((candidate) => candidate.id === detail.value.projectId);
      if (!project)
        return yield* validation(
          "invalid-protocol-state",
      ...
      return {
        thread: detail.value,
        cwd: detail.value.worktreePath ?? project.workspaceRoot,
      };
    });

  // Load complete stored results only when listing or resolving evidence; metadata
  // callers and turns with persisted artifacts do not need to decode them again.
  const loadCurrentTurnEvidence = Effect.fn("MagiService.loadCurrentTurnEvidence")(function* (
    thread: OrchestrationThread,
  ) {
    const activities = thread.latestTurn
    ...
    return {
      ...thread,
      activities: activities.map(({ activityId, ...activity }) => ({
        ...activity,
        id: activityId,
      })),
    };
  ...
  const listContextActivities = (threadId: ThreadId) =>
    Effect.gen(function* () {
      ...
      return { activities: listMagiContextActivities(yield* loadCurrentTurnEvidence(root.thread)) };
    });
      ...
      const providerFiber = yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          const capacity = yield* resolveMagiPromptCapacity({
            ...
            compact: providerService
              .compactThread(input.member.threadId, input.member.participant.modelSelection)
              .pipe(
                Effect.as(true),
      ...
      const activities =
        protocolBeforeWork.pendingContextArtifacts.length > 0
          ...
          : yield* resolveMagiContextActivities({
              thread: yield* loadCurrentTurnEvidence(root.thread),
  ...
  const listRuns = (input: MagiListRunsInput) =>
    Effect.gen(function* () {
      const ownerThreadIds = input.includeDescendants
        ? magiHistoryOwners(input.rootThreadId, (yield* snapshots.getShellSnapshot()).threads)
        : [input.rootThreadId];
      return yield* repository.listRuns({ ...input, ownerThreadIds });
    }).pipe(
  ...
  yield* MagiControlBroker.installActiveHandlers({
      ...
      resolveMagiInvocation(scope, {
        getBinding: (threadId) =>
          Option.isSome(sessionDirectory)
            ? sessionDirectory.value.getBinding(threadId)
            : Effect.succeed(Option.none()),
        getThreads: () =>
          snapshots.getShellSnapshot().pipe(Effect.map((snapshot) => snapshot.threads)),
      }),
    ...
    controlRun: (scope, input) => service.controlRun(scope.threadId, input),
...
export const layer = Layer.effect(MagiService, makeMagiService).pipe(
  Layer.provide([ProjectionMagiRepositoryLive, ProjectionThreadActivityRepositoryLive]),
);
