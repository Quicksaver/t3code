...
This split branch is synchronized through `upstream/main` `cabc93bad305c14e1b6919c6631deca153032f9f` (`Make the thread composer read as elevated liquid glass (#3668)`) as of 2026-07-03. The merge was generated from branch head `13c415d1e4a2edb135a8798db59ed949309461bc`, current-root reference `530239fc627659cb7900c69582bf520ed172555e`, prior fork merge baseline upstream `7b9eef7ac`, and local code-pick reference `72ef94989`. The branch-specific implementation source of truth remains this file; root `CUSTOMIZED.md` is intentionally absent in this isolated branch.

At this audited 2026-07-03 snapshot, topic commit `712a9af1ccdbacad13e714efc4ee35a9b70b8180` is 26 commits ahead and 0 commits behind `upstream/main`. Including the current Step 3 composer-overlay width cleanup and this documentation refresh in the working tree, the branch diff against `upstream/main` was 126 files changed, 16518 insertions(+), and 490 deletions(-). Treat those file/change counts as a point-in-time audit aid; recompute them from Git for later branch heads.

The 2026-07-03 merge preserved the VS Code extension architecture, visible VS Code sidebar trigger, desktop/sidebar compatibility, host display preferences, local extension packaging, persistence cleanup coverage, Sonnet 5 model helper coverage, and the conversation-width policy for timeline rows, composer banners, composer, composer overlay blur chrome, branch toolbar, and minimap gutter decisions. Incoming upstream changes were accepted for the Electron dev launcher fix, adaptive iPad/mobile workspace split-view layout, EAS native pod source compilation, and elevated liquid-glass composer styling. The upstream liquid-glass composer work did not retire the branch's width helper: the composer form still applies `resolveThreadConversationMaxWidthStyle(...)` around the upstream glass surface, and the Step 3 follow-up applies the same helper to the composer overlay blur chrome in `apps/web/src/components/ChatView.tsx`. The VS Code `t3code.ui.threadConversationMaxWidth` setting remains an explicit opt-in max-width override with empty/null meaning no maximum width.

The 2026-07-03 merge had one textual conflict in `pnpm-lock.yaml`. The resolution used upstream's lockfile side as the base, then regenerated the lockfile from the merged manifests with `pnpm install --lockfile-only --ignore-scripts` so upstream's mobile Expo resolutions stayed authoritative while the branch-only VS Code extension importer and `tsdown` catalog entry remained present. No branch customization was retired or made obsolete by this merge; no new fork-only conflict-resolution code was introduced outside the lockfile and this snapshot note.

The 2026-06-30 merge preserved the VS Code extension architecture, desktop-backed webview bootstrap, workspace-folder identity sharing through `@t3tools/shared/workspaceFolders`, direct host-injected primary environment registration from `window.t3HostBridge.getLocalEnvironmentBootstrap()`, host MCP discovery diagnostics, release packaging, focused extension/web/server coverage, and VS Code host display preferences for hidden source-control, terminal, and conversation-width surfaces. Upstream's preview-browser automation hardening, browser recording stability work, desktop backend pool, parallel WSL/Windows local backends, desktop mode picker, desktop-local connection catalog entries, shared HTTP readiness helper, chat scroll affordance restoration, timeline scroll anchoring helpers, and timeline minimap were accepted as the baseline. No VS Code extension customization was retired by this merge.

The 2026-07-01 merge preserved the same VS Code extension architecture and webview display preferences while accepting upstream's WSL-only backend warmup before preflight, middle-click close behavior for right-panel tabs, Claude Sonnet 5 as the default Claude model with updated Sonnet aliases, and the restored ultrathink frame border effect. The merge had no textual conflicts. These upstream changes do not retire any VS Code extension customization: host-managed conversation width still composes with upstream timeline/minimap behavior, VS Code-hidden right-panel surfaces remain hidden when configured even though visible tabs now support middle-click close, desktop-owned backend discovery continues to use the primary advertisable backend while upstream warms selected WSL environments earlier, and provider sessions keep host MCP injection while adopting the new Claude default model catalog.

The 2026-06-30 merge had no textual conflicts. The branch-only VS Code extension package version was aligned to upstream release `0.0.28`, matching the branch's release-version script coverage for `apps/vscode-extension/package.json`, and the release-version test now guards that the branch-only extension manifest remains part of future release version bumps. The VS Code webview keeps composing upstream's restored chat scroll controls and timeline minimap with host-managed workspace behavior: display preferences still hide VS Code-duplicated chrome such as the Version Control panel and embedded terminal where configured, while upstream chat anchoring remains shared web UI behavior. The Step 3 follow-up adapted the minimap gutter calculation to use the effective host conversation width in VS Code instead of always assuming upstream's default `768px` content width, so fixed-width and no-cap VS Code settings do not make the minimap persist in the wrong gutter.
...
The first five settings default to `false`; `t3code.ui.threadConversationMaxWidth` defaults to an empty value, which applies no maximum width to the React app's conversation and prompt input surfaces. Values are passed to the React app through `window.t3HostBridge.getDisplayPreferences()` at startup and through `window.t3HostBridge.onDisplayPreferencesChanged(...)` while the webview is open, so changes apply without reopening the T3 Code view. In VS Code webviews, the effective host conversation width is applied consistently to timeline rows, the composer form, composer overlay blur chrome, composer banners, and the branch toolbar. The upstream timeline minimap keeps the normal web/desktop `768px` content-width assumption outside VS Code, but inside VS Code its persistent-gutter decision uses the host-provided conversation width; when the VS Code setting is empty/no-cap, the minimap treats the content as full-width and stays hover/focus-only instead of reserving a persistent gutter. When `t3code.ui.enableTerminal` is `false`, the embedded T3 terminal drawer is disabled, terminal keybindings are ignored, terminal-backed project actions are hidden, and any open terminal drawer is closed. When `t3code.ui.enableSourceControlPanel` is `false`, the Version Control right-panel surface is hidden inside VS Code webviews.
...
- The web app receives the environment id, label, backend URLs, and bearer token through `window.t3HostBridge`, so it can register the primary environment and authenticate without depending on VS Code webview cookies or a descriptor fetch before sidebar startup.
