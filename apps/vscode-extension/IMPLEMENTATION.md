...
After the 2026-07-03 upstream merge and this documentation refresh are committed, `split/vscode-extension-work` is expected to be 26 commits ahead and 0 commits behind `upstream/main`. The branch diff against `upstream/main` is 126 files changed, 16513 insertions(+), and 489 deletions(-).

The 2026-07-03 merge preserved the VS Code extension architecture, visible VS Code sidebar trigger, desktop/sidebar compatibility, host display preferences, local extension packaging, persistence cleanup coverage, Sonnet 5 model helper coverage, and the conversation-width policy for timeline rows, composer banners, composer, branch toolbar, and minimap gutter decisions. Incoming upstream changes were accepted for the Electron dev launcher fix, adaptive iPad/mobile workspace split-view layout, EAS native pod source compilation, and elevated liquid-glass composer styling. The upstream liquid-glass composer work did not retire the branch's width helper: the composer form still applies `resolveThreadConversationMaxWidthStyle(...)` around the upstream glass surface, and the VS Code `t3code.ui.threadConversationMaxWidth` setting remains an explicit opt-in max-width override with empty/null meaning no maximum width.
