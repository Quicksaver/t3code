...
function resolveHostPrimaryTarget(): PrimaryEnvironmentTarget | null {
  ...
  if (!hostBootstrap.httpBaseUrl || !hostBootstrap.wsBaseUrl) {
    throw new DesktopEnvironmentBootstrapIncompleteError({
      hasHttpBaseUrl: Boolean(hostBootstrap.httpBaseUrl),
      hasWsBaseUrl: Boolean(hostBootstrap.wsBaseUrl),
    });
  }
  ...
  return {
    ...
    target: {
      httpBaseUrl: normalizeBaseUrl(hostBootstrap.httpBaseUrl, "desktop-managed", "http-base-url"),
      wsBaseUrl: normalizeBaseUrl(hostBootstrap.wsBaseUrl, "desktop-managed", "websocket-base-url"),
    },
  };
}
