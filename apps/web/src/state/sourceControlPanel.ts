...
export function useSourceControlPanelApi(environmentId: EnvironmentId) {
  ...
  const runFederatedPanelCommand = useCallback(
    async <TInput extends { readonly cwd: string }, TResult>(
      ...
      return unwrapPanelCommand(await command({ environmentId: targetEnvironmentId, input }));
    },
    [],
  );
  ...
  return useMemo(
    () => ({
      ...
      federatedVcs: {
        ...
        fetchBranch: (
          targetEnvironmentId: EnvironmentId,
          input: Parameters<typeof panelFetchBranch>[0]["input"],
        ) =>
          runFederatedPanelCommand<typeof input, void>(
            targetEnvironmentId,
            panelFetchBranch,
            input,
          ),
        pullBranch: (
          targetEnvironmentId: EnvironmentId,
          input: Parameters<typeof panelPullBranch>[0]["input"],
        ) =>
          runFederatedPanelCommand<typeof input, unknown>(
            targetEnvironmentId,
            panelPullBranch,
            input,
          ),
