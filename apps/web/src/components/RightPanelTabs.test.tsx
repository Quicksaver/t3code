...
describe("surface shortcuts", () => {
  it("keeps Device and Magi independently reachable in the shared launcher", () => {
    ...
    expect(surfaceShortcutActionForKey(actions, shortcutEvent("m"))?.id).toBe("device");
    expect(surfaceShortcutActionForKey(actions, shortcutEvent("g"))).toMatchObject({
      id: "magi",
      badgeCount: 2,
    });
    expect(actions.at(-1)?.id).toBe("source-control");
