...
describe("RightPanelTabs add-surface actions", () => {
  it("keeps Magi keyboard activation and its live badge alongside Version Control", () => {
    const onAddMagi = vi.fn();
    const actions = buildAddSurfaceActions(
      { ...actionProps(), onAddMagi, magiAvailable: true, liveMagiRunCount: 2 },
      "menu",
    );
    const action = surfaceShortcutActionForKey(actions, shortcutEvent("g"));
    expect(action).toMatchObject({ id: "magi", badgeCount: 2, instancePolicy: "singleton" });
    action?.onClick();
