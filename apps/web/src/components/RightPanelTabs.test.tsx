...
describe("RightPanelTabs add-surface actions", () => {
  ...
  it.each(["empty-state", "menu"] as const)(
    "preserves activity counts in %s actions",
    (placement) => {
      const actions = buildAddSurfaceActions({ ...actionProps(), liveMagiRunCount: 1 }, placement);

      expect(actions.find((action) => action.id === "agents")?.badgeCount).toBe(3);
      expect(actions.find((action) => action.id === "magi")?.badgeCount).toBe(1);
