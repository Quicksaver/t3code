...
function ChatViewContent(props: ChatViewProps) {
  ...
  const activeTimelineAnchorEndIndexRef = useRef<number | null>(null);
  ...
  const liveFollowSecondFrameRef = useRef<number | null>(null);
  const cancelTimelineLiveFollowForUserNavigation = useCallback(() => {
    ...
    activeTimelineAnchorEndIndexRef.current = null;
  ...
  const cancelScheduledLiveFollow = useCallback(() => {
    if (liveFollowFrameRef.current !== null) {
      cancelAnimationFrame(liveFollowFrameRef.current);
      liveFollowFrameRef.current = null;
    }
    if (liveFollowSecondFrameRef.current !== null) {
      cancelAnimationFrame(liveFollowSecondFrameRef.current);
      liveFollowSecondFrameRef.current = null;
    }
  }, []);
  ...
  const getActiveTimelineTurnMetrics = useCallback(
    (list?: LegendListRef | null) => {
      ...
      return getAnchoredTurnMetrics({
        ...
        anchorIndex,
        ...(activeTimelineAnchorEndIndexRef.current !== null
          ? { endIndex: activeTimelineAnchorEndIndexRef.current }
          : {}),
  ...
  useEffect(() => () => showScrollDebouncer.current.cancel(), []);
  useEffect(() => {
    ...
    const frame = requestAnimationFrame(() => {
      ...
      const handlePointerManualNavigation = (event: PointerEvent) => {
      ...
      scrollNode.addEventListener("pointerdown", handlePointerManualNavigation, {
        passive: true,
      });
      removeListeners = () => {
        scrollNode.removeEventListener("wheel", handleManualNavigation);
        scrollNode.removeEventListener("touchmove", handleManualNavigation);
        scrollNode.removeEventListener("pointerdown", handlePointerManualNavigation);
      };
    });
  ...
  const onTimelineAnchorReady = useCallback(
    (messageId: MessageId, anchorIndex: number, anchorEndIndex: number) => {
      if (pendingTimelineAnchorRef.current === messageId) {
        pendingTimelineAnchorRef.current = null;
      }
      activeTimelineAnchorIndexRef.current = anchorIndex;
      activeTimelineAnchorEndIndexRef.current = anchorEndIndex;
      if (positionedTimelineAnchorRef.current === messageId) {
        return;
      }
  ...
  const scheduleLiveFollowScroll = useCallback(() => {
    if (
      !activeThread?.id ||
      liveFollowFrameRef.current !== null ||
      liveFollowSecondFrameRef.current !== null
    ...
    liveFollowFrameRef.current = requestAnimationFrame(() => {
      liveFollowFrameRef.current = null;
      liveFollowSecondFrameRef.current = requestAnimationFrame(() => {
        liveFollowSecondFrameRef.current = null;
        if (liveFollowUserScrollGenerationRef.current !== anchorUserScrollGenerationRef.current) {
          return;
        }
  ...
  useEffect(() => {
    scheduleLiveFollowScroll();
  }, [scheduleLiveFollowScroll, timelineEntries]);

  useEffect(() => {
    return cancelScheduledLiveFollow;
  }, [activeThread?.id, cancelScheduledLiveFollow]);
