...
import { SettingsListDetailFrame } from "./SettingsListDetail";
import {
...
export function EnvironmentSettingsPlaceholder({
  sectionTitle,
  title,
  description,
  environmentTabs,
  ...
  return (
    <SettingsSection title={sectionTitle} hideTitle variant="plain">
      {environmentTabs ? (
        <div className="flex min-h-11 min-w-0 items-center px-3 sm:px-4">{environmentTabs}</div>
      ) : null}
      <SettingsListDetailFrame className="flex overflow-x-hidden overflow-y-auto">
...
export function EnvironmentSettingsPanel({
  ...
  const effectiveEnvironmentId =
    scoped || targetEnvironmentMissing
      ? targetEnvironmentId
  ...
  const environmentTabs = scoped ? null : (
    <EnvironmentTabs
  ...
  return (
    <SettingsPageContainer
      className="@container/settings-list-detail gap-8"
    ...
    >
      {targetEnvironmentMissing ? (
        <EnvironmentSettingsPlaceholder
          ...
          description={targetUnavailableDescription}
        />
      ) : null}
