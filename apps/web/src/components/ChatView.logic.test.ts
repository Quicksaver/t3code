...
describe("buildRunningThreadTurnInterruptInput", () => {
  it("uses child lifecycle and turn ownership for the stop shortcut", () => {
    const childTurnId = TurnId.make("child-turn");
    const child = makeThread({
      parentRelation: makeSubagentParentRelation(),
      latestTurn: { ...completedTurn, turnId: childTurnId, state: "running", completedAt: null },
      session: readySession,
    });
    expect(buildRunningThreadTurnInterruptInput(child, "ready")).toEqual({
      threadId,
      turnId: childTurnId,
    });
    expect(buildRunningThreadTurnInterruptInput({ ...child, latestTurn: null }, "ready")).toEqual({
      threadId,
    });
    expect(
      buildRunningThreadTurnInterruptInput(
        {
          ...child,
          parentRelation: makeSubagentParentRelation({ status: "completed", completedAt: now }),
        },
        "running",
      ),
    ).toBeNull();
...
describe("hasServerAcknowledgedLocalDispatch", () => {
  ...
  it("accepts an exact projection while the provider session is starting", () => {
    const localDispatch = createLocalDispatchSnapshot(
      makeThread({ latestTurn: completedTurn, session: readySession }),
      expectedUserMessageId,
    );
    ...
    expect(
      hasServerAcknowledgedLocalDispatch({
        ...
        projectedMessages: [{ id: expectedUserMessageId, role: "user" }],
        hasPendingApproval: false,
        hasPendingUserInput: false,
        threadError: null,
      }),
  ...
  it("acknowledges only a new turn-start failure", () => {
    const expectedUserMessageId = MessageId.make("message-turn-start-failure");
    const localDispatch = {
      ...createLocalDispatchSnapshot(makeThread(), expectedUserMessageId),
