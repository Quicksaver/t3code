...
export interface AddPanelSurfaceActionProps {
  ...
  readonly liveMagiRunCount?: number;
}
...
interface AddPanelSurfaceDescriptor {
  ...
  readonly badge: "live-agents" | "live-magi" | null;
}
...
export const ADD_PANEL_SURFACE_DESCRIPTORS = [
  {
    id: "magi",
    label: "Magi",
    description: "Run weighted agent consensus.",
    icon: Network,
    shortcut: "G",
    availability: "magiAvailable",
    activation: "onAddMagi",
    instancePolicy: "singleton",
    order: { "empty-state": 9, menu: 7.5 },
  ...
  },
  {
    ...
    description: "Open this thread's linked pull requests.",
...
export function buildAddSurfaceActions(
  ...
  return ADD_PANEL_SURFACE_DESCRIPTORS.map((descriptor) => ({
    ...
    badgeCount:
      descriptor.badge === "live-agents"
        ? props.liveAgentCount
        : descriptor.badge === "live-magi"
          ? (props.liveMagiRunCount ?? 0)
          : 0,
