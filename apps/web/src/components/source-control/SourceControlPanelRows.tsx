  ...
  return (
    ...
    >
      <div className="flex h-8 shrink-0 items-center justify-between gap-2 px-2">
        <button
          type="button"
          className="flex min-w-0 cursor-pointer items-center gap-1.5 text-left text-xs font-semibold uppercase tracking-normal text-muted-foreground hover:text-foreground"
...
export function LoadMoreCommitsButton({
  ...
  return (
    <button
      type="button"
      className="flex h-7 w-full cursor-pointer items-center rounded px-1.5 text-left text-xs text-muted-foreground hover:bg-accent/60 hover:text-foreground disabled:cursor-default"
      disabled={loading}
      onClick={onClick}
    >
      {loading
