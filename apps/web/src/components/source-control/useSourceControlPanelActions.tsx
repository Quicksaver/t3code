...
export function useSourceControlPanelActions(
  ...
  const publishBranch = useCallback(
    ...
    [cwd, pushBranch, runAction, snapshot],
  );
  ...
  const publishBranchWithRemoteChoice = useCallback(
    (branch: VcsRef, force = false) => {
      ...
      if (branchNeedsRepositoryPublish(branch, snapshot)) {
        onPublishRepository(branchOperationCwd(branch, cwd));
        return;
      }
    ...
    [cwd, onPublishRepository, publishBranch, snapshot],
  );
  ...
  const runBranchSync = useCallback(
    (
      ...
      if (!branch.current) {
        ...
        void runAction(actionKey, async () => {
          ...
          if (state === "push") {
            await pushBranch(branch, snapshot, { cwd: targetCwd, force });
            return;
          }
          if (state === "pull") {
      ...
      void runAction(`branch-sync:${branch.name}`, async () => {
        ...
        if (aheadCount > 0) {
          await pushBranch(branch, syncSnapshot, { cwd: targetCwd, force });
          return;
        }
        if (behindCount > 0) {
          await api.vcs.pullBranch({
    ...
    [api, cwd, publishBranchWithRemoteChoice, pushBranch, runAction, snapshot],
  );
  ...
  const runDivergedSync = useCallback(
    (mode: "force-pull" | "merge" | "force-push") => {
      ...
      void runAction(`branch-sync:${branch.name}`, async () => {
        ...
        if (mode === "force-push") {
          if (!snapshot) return;
          await pushBranch(branch, snapshot, { cwd: targetCwd, force: true });
          return;
        }
        if (mode === "force-pull") {
        ...
        if (!snapshot) return;
        await pushBranch(branch, snapshot, { cwd: targetCwd });
      });
    },
    [api, cwd, divergedSyncBranch, pushBranch, runAction, snapshot],
  );

  const fetchActionableBranches = useCallback(
    (force = false) =>
      runAction("work-fetch", async () => {
