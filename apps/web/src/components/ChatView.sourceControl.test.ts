...
describe("buildSourceControlEnvironmentOption", () => {
  it("builds one option shared by the environment picker and federated Source Control", () => {
    const projectId = ProjectId.make("project-remote");
    const project = {
      environmentId: EnvironmentId.make("environment-remote"),
      id: projectId,
      workspaceRoot: "/repos/project-remote",
      scripts: [],
    };
...
describe("source control right panel surface visibility", () => {
  ...
  it("hides unavailable Source Control surfaces without affecting other surfaces", () => {
    expect(
      normalizeSourceControlRightPanelPresence({
        activeSurface: agentsSurface,
        sourceControlAvailable: false,
        surfaces: [sourceControlSurface, agentsSurface],
      }),
    ).toEqual({ activeSurface: agentsSurface, surfaces: [agentsSurface] });
  ...
  it("keeps pull-request tabs visible when Source Control becomes unavailable", () => {
    ...
    expect(visiblePresence).toEqual({
      ...
      surfaces: [pullRequestSurface, agentsSurface],
    });
  });
...
describe("runSourceControlServerMetadataUpdate", () => {
  ...
  it("retains predecessor guards when an in-flight chained write observes its input", async () => {
    const queue = createSourceControlServerMetadataUpdateQueue();
    const calls: Array<{ expectedBranch: string | null; branch: string | null }> = [];
  ...
  it("accepts an observed request target that is also a snapshotted predecessor", async () => {
    const queue = createSourceControlServerMetadataUpdateQueue();
    const calls: Array<{ expectedBranch: string | null; branch: string | null }> = [];
    let releaseSecond: (() => void) | undefined;
    const updateThreadMetadata = async (
