 ...
 * empty chat composer is still where the user's next keystroke belongs.
 * Excluding contenteditable="false" lets closest find an editable ancestor.
 */
          ...
          {actions.map((action) =>
            action.available ? (
              ...
              >
                ...
                <span className="mt-1.5 text-muted-foreground text-xs leading-relaxed">
                  {action.unavailableHint ?? action.disabledReason}
                </span>
              </div>
            ),
          )}
