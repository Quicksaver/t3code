...
function AgentRow({ agent, depth = 0 }: { agent: RuntimeSubagent; depth?: number }) {
  ...
  const content = (
    <div
      className={cn(
        "grid grid-cols-[0.375rem_minmax(0,1fr)_auto] grid-rows-[1.25rem_1.125rem_1rem] items-center gap-x-2 rounded-md",
        onOpen
          ? "h-[4.375rem] px-[var(--sidebar-row-content-inset)] py-[var(--sidebar-content-inset)]"
          : "h-[3.875rem] px-1.5 py-1",
      )}
    >
      ...
      <span className="col-start-2 row-start-1 flex min-w-0 items-baseline gap-2">
        <span
          className={cn(
            ...
            onOpen && settled ? "font-normal text-secondary-label" : "font-medium",
          )}
        >
          {title}
        </span>
        {role ? (
          <span
            className={cn(
              "max-w-28 shrink-0 truncate rounded-sm border px-1",
              onOpen
                ? "border-border text-[11px] text-secondary-label"
                : "border-border/60 font-mono text-[.65rem] text-muted-foreground",
            )}
          >
            {role}
          </span>
      ...
      <span
        className={cn(
          "col-start-3 row-start-1 min-w-14 text-right",
          onOpen
            ? "text-xs text-secondary-label"
            : "font-mono text-[.7rem] text-muted-foreground/80",
        )}
      >
      ...
      <span
        className={cn(
          "col-start-2 col-end-4 row-start-3 truncate tabular-nums",
          onOpen
            ? "text-xs text-secondary-label"
            : "font-mono text-[.7rem] text-muted-foreground/70",
        )}
      >
        {metadata.join(" · ")}
      </span>
  ...
  return (
    <div style={{ paddingLeft: `${Math.min(Math.max(depth, 0), 6) * 0.75}rem` }}>
      {onOpen ? (
        <button
          type="button"
          className="w-full cursor-pointer overflow-hidden rounded-md bg-transparent text-left text-foreground outline-none select-none hover:bg-accent focus-visible:bg-accent focus-visible:ring-2 focus-visible:ring-ring active:bg-accent"
          onClick={onOpen}
