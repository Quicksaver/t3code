import type { MagiListRunsResult, MagiRunSummary, ScopedThreadRef } from "@t3tools/contracts";
...
export function useMagiRunHistory(input: {
  ...
  useEffect(
    () =>
      startMagiHistoryLiveRefresh({
        ...
        refresh: () => {
          appAtomRegistry.refresh(magiEnvironment.history(expandedTarget));
          // The owner may be outside the descendant page; keep its timeline summary live too.
    ...
    [expandedTarget, input.expanded, latestTarget],
  );
  ...
  return {
    ...
    // The panel may contain 100 newer child runs; its ordering cannot select the owner's timeline.
    latestOwnedRun:
      latestHistory?.runs.find((run) => run.rootThreadId === input.threadRef.threadId) ?? null,
  };
}
