...
// Thread snapshots written before v5 may embed large command outputs. Clear
// only this disposable cache on upgrade so the first read uses the compact
// server projection instead of hydrating an oversized legacy payload.
// Live events in v5 could lose their deferred-output marker before being cached.
