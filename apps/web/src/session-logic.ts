...
function compareActivitiesByOrder(
  ...
  if (
    shouldPreserveSameTimestampToolUpdateOrder(left, right) ||
    (asRecord(left.payload)?.itemType === "collab_agent_tool_call" &&
      asRecord(right.payload)?.itemType === "collab_agent_tool_call")
  ) {
    return 0;
  }
...
function isParentTimelineWorkEntry(entry: WorkLogEntry): boolean {
  return entry.itemType !== "collab_agent_tool_call" || (entry.subagentChildren?.length ?? 0) === 0;
}
...
export function deriveTimelineEntriesWithState(
  ...
  if (canAppend) {
    ...
    const workRows = workEntries
      .slice(previous.workEntries.length)
      .filter(isParentTimelineWorkEntry)
      .map(timelineEntryFromWork);
  ...
  const workRows = workEntries.filter(isParentTimelineWorkEntry).map(timelineEntryFromWork);
  return {
    messages,
    proposedPlans,
    workEntries,
