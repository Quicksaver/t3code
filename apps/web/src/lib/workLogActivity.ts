...
import {
  ...
  type ExtractedToolActivityPresentation,
...
export interface ParsedWorkLogActivityPayload extends ExtractedToolActivityPresentation {
...
function extractCommandResult(
  ...
  const rawOutputStdout = options.preserveBlankRawOutputStreams
    ? firstRawStringFromRecord(rawOutput, WORK_LOG_COMMAND_STDOUT_KEYS)
    : firstCommandOutputStringFromRecord(rawOutput, WORK_LOG_COMMAND_STDOUT_KEYS);
  const stdout =
    rawOutputStdout ??
    firstCommandOutputStringFromRecord(itemResult, WORK_LOG_COMMAND_STDOUT_KEYS) ??
    firstCommandOutputStringFromRecord(data, WORK_LOG_COMMAND_STDOUT_KEYS) ??
    firstCommandOutputStringFromRecord(payload, WORK_LOG_COMMAND_STDOUT_KEYS);
  const stderr =
    (options.preserveBlankRawOutputStreams
      ? firstRawStringFromRecord(rawOutput, WORK_LOG_COMMAND_STDERR_KEYS)
      : firstCommandOutputStringFromRecord(rawOutput, WORK_LOG_COMMAND_STDERR_KEYS)) ??
    firstCommandOutputStringFromRecord(itemResult, WORK_LOG_COMMAND_STDERR_KEYS) ??
    firstCommandOutputStringFromRecord(data, WORK_LOG_COMMAND_STDERR_KEYS) ??
    firstCommandOutputStringFromRecord(payload, WORK_LOG_COMMAND_STDERR_KEYS);
  const rawOutputContent = options.preserveBlankRawOutputStreams
    ? firstRawStringFromRecord(rawOutput, WORK_LOG_COMMAND_CONTENT_KEYS)
    : firstCommandOutputStringFromRecord(rawOutput, WORK_LOG_COMMAND_CONTENT_KEYS);
  const content =
    stdout ??
    rawOutputContent ??
    firstCommandOutputStringFromRecord(itemResult, WORK_LOG_COMMAND_CONTENT_KEYS) ??
    firstCommandOutputStringFromRecord(item, WORK_LOG_COMMAND_ITEM_CONTENT_KEYS);
  ...
  const exitCode =
    firstIntegerFromRecord(rawOutput, WORK_LOG_COMMAND_EXIT_CODE_KEYS) ??
    firstIntegerFromRecord(itemResult, WORK_LOG_COMMAND_EXIT_CODE_KEYS) ??
    firstIntegerFromRecord(item, WORK_LOG_COMMAND_EXIT_CODE_KEYS) ??
    firstIntegerFromRecord(data, WORK_LOG_COMMAND_EXIT_CODE_KEYS) ??
    firstIntegerFromRecord(payload, WORK_LOG_COMMAND_EXIT_CODE_KEYS) ??
    strippedContent?.exitCode ??
    detailExit?.exitCode ??
    null;
  const elapsedSeconds =
    firstNumberFromRecord(rawOutput, WORK_LOG_COMMAND_ELAPSED_SECONDS_KEYS) ??
    firstNumberFromRecord(itemResult, WORK_LOG_COMMAND_ELAPSED_SECONDS_KEYS) ??
    firstNumberFromRecord(item, WORK_LOG_COMMAND_ELAPSED_SECONDS_KEYS) ??
    firstNumberFromRecord(data, WORK_LOG_COMMAND_ELAPSED_SECONDS_KEYS) ??
    firstNumberFromRecord(payload, WORK_LOG_COMMAND_ELAPSED_SECONDS_KEYS);
  const durationMs =
    firstNumberFromRecord(rawOutput, WORK_LOG_COMMAND_DURATION_MS_KEYS) ??
    firstNumberFromRecord(itemResult, WORK_LOG_COMMAND_DURATION_MS_KEYS) ??
    firstNumberFromRecord(item, WORK_LOG_COMMAND_DURATION_MS_KEYS) ??
    firstNumberFromRecord(data, WORK_LOG_COMMAND_DURATION_MS_KEYS) ??
    firstNumberFromRecord(payload, WORK_LOG_COMMAND_DURATION_MS_KEYS) ??
  ...
  let normalizedStdout = strippedStdout?.exitCode !== undefined ? strippedStdout.output : stdout;
  let normalizedStderr = stderr;

  if (
    rawOutput?.truncated === true &&
    ![normalizedOutput, normalizedStdout, normalizedStderr].some((value) =>
      value?.endsWith(WORK_LOG_COMMAND_OUTPUT_TRUNCATED_MARKER),
    )
  ) {
    if (normalizedStderr !== null) {
      normalizedStderr += WORK_LOG_COMMAND_OUTPUT_TRUNCATED_MARKER;
    } else if (normalizedStdout !== null) {
      const originalStdout = normalizedStdout;
      normalizedStdout += WORK_LOG_COMMAND_OUTPUT_TRUNCATED_MARKER;
      if (normalizedOutput === originalStdout) {
        normalizedOutput = normalizedStdout;
      }
    } else if (normalizedOutput !== null) {
      normalizedOutput += WORK_LOG_COMMAND_OUTPUT_TRUNCATED_MARKER;
    }
  }
...
function extractToolCallId(payload: Record<string, unknown> | null): string | null {
  ...
  return (
    ...
    (payload?.itemType === "collab_agent_tool_call"
      ? (asTrimmedString(asRecord(data?.parentCollab)?.itemId) ??
...
export function normalizePreviewForComparison(value: string | null | undefined): string | null {
...
function collectPatchStrings(
  ...
  if (
    depth > WORK_LOG_ACTIVITY_LIMITS.maxSearchDepth ||
    patches.length >= WORK_LOG_ACTIVITY_LIMITS.maxPatches
  ) {
    return;
  }
  if (Array.isArray(value)) {
    for (const entry of value) {
      ...
      if (patches.length >= WORK_LOG_ACTIVITY_LIMITS.maxPatches) {
        return;
      }
    }
    return;
  ...
  for (const key of WORK_LOG_PATCH_KEYS) {
    ...
    if (candidate.length > WORK_LOG_ACTIVITY_LIMITS.maxPatchChars) {
      seen.add(candidate);
      continue;
    }
...
function collectChangedFiles(value: unknown, target: string[], seen: Set<string>, depth: number) {
  if (
    depth > WORK_LOG_ACTIVITY_LIMITS.maxSearchDepth ||
    target.length >= WORK_LOG_ACTIVITY_LIMITS.maxChangedFiles
  ) {
    return;
  }
  if (Array.isArray(value)) {
    for (const entry of value) {
      ...
      if (target.length >= WORK_LOG_ACTIVITY_LIMITS.maxChangedFiles) {
        return;
      }
    }
    return;
