...
interface WorkEntryDetailContext {
  visibleLabel?: string;
...
function buildGenericToolExpandedBody(
  ...
  { visibleLabel, viewedImagePath }: WorkEntryDetailContext,
): string | null {
  const blocks: string[] = [];
  ...
  const command = workEntry.command?.trim();
  if (!command || command !== visibleLabel?.trim()) {
    addBlock(deriveRawCommand(workEntry) ?? command);
  }
  const detail = workEntry.detail?.trim();
...
export function hasExpandableWorkEntryDetails(workEntry: WorkLogEntry): boolean {
  return hasRichWorkEntryDetails(workEntry) || hasGenericToolExpandedBody(workEntry);
}

/** Rich command/file rows bypass the generic grouped-tool summary. */
export function hasRichWorkEntryDetails(workEntry: WorkLogEntry): boolean {
...
export function deriveExpandableWorkEntryDetails(
  workEntry: WorkLogEntry,
  workspaceRoot: string | undefined,
  context: WorkEntryDetailContext = {},
  ...
  const genericDetail = buildGenericToolExpandedBody(workEntry, workspaceRoot, context);
