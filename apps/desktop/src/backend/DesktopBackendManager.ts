...
const { logWarning: logBackendManagerWarning } =
...
export interface BackendInstanceSpec {
  ...
  // Only desktop-owned primary-like instances should advertise a local
  // backend endpoint for VS Code discovery. Secondary desktop-local backends
  // such as WSL are reachable through desktop IPC and must not write primary
  // desktop-backend advertisements.
...
}): Option.Option<DesktopBackendAdvertisementHandle> {
  const t3Home = input.config.bootstrap.t3Home;
  return t3Home === undefined
    ? Option.none()
    : Option.some({
        t3Home,
      ...
      Effect.gen(function* () {
        ...
        const program = runBackendProcess({
          ...
          onReady: Effect.fn("desktop.backendInstance.onReady")(function* () {
            ...
            const advertisement =
              spec.advertiseDesktopBackend === true
                ? makeDesktopBackendAdvertisementHandle({
                    runId,
                    config: config.value,
                  })
                : Option.none<DesktopBackendAdvertisementHandle>();
            const advertisementReady = yield* Option.match(advertisement, {
            ...
            yield* Option.match(advertisement, {
              onNone: () => Effect.void,
              onSome: (handle) =>
                Effect.repeat(
                  refreshDesktopBackendAdvertisement(handle, config.value),
