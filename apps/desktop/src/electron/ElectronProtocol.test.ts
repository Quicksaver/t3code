...
describe("ElectronProtocol", () => {
  ...
  it.effect("retargets the stable renderer origin after registration", () =>
    Effect.gen(function* () {
      let handler: ((request: Request) => Promise<Response>) | undefined;
      handleMock.mockImplementation((_scheme, nextHandler) => {
        handler = nextHandler;
      ...
      yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          yield* protocol.setDesktopProtocolTargetOrigin(new URL("http://172.27.0.99:3773/"));
          yield* Effect.promise(() => handler!(new Request("t3code://app/assets/index.js")));
        }),
      );

      assert.equal(netFetchMock.mock.calls[0]?.[0], "http://172.27.0.99:3773/assets/index.js");
    }).pipe(Effect.provide(ElectronProtocol.layer)),
  );
