...
type ThreadListV2LifecycleScope = {
...
/** Rendering and movement share the same lifecycle partitions before disclosure. */
function partitionThreadListV2Lineage(
  input: ThreadListV2LifecycleScope & {
  ...
  for (const thread of input.threads) {
    const depth = input.threadDepths.get(threadShellKey(thread)) ?? 0;
...
function pinnedLineageRepresentative(group: LineageGroup) {
  return group.find((thread) => thread.pinnedAt != null) ?? group[0];
}

function canonicalThreadListV2Groups(
  input: ThreadListV2LifecycleScope & { readonly threads: readonly EnvironmentThreadShell[] },
) {
  ...
  return partitionThreadListV2Lineage({
    ...input,
    threads: lineage.threads,
    threadDepths: lineage.threadDepths,
  });
}

/** Root menus and dispatch resolve the same canonical movement atom. Pin
 * decoration and selected-terminal navigation never choose the write target. */
export function resolveThreadListV2OrderingTargets(
  input: ThreadListV2LifecycleScope & { readonly threads: readonly EnvironmentThreadShell[] },
) {
  const { activeGroups, pinnedGroups } = canonicalThreadListV2Groups(input);
  const targets = new Map<
    string,
    { readonly section: "active" | "pinned"; readonly representativeKey: string }
  >();
  for (const section of ["active", "pinned"] as const) {
    for (const group of section === "active" ? activeGroups : pinnedGroups) {
      const root = group[0];
      if (!canUseThreadListV2LifecycleActions(root)) continue;
      targets.set(threadShellKey(root), {
        section,
        representativeKey: threadShellKey(
          section === "pinned" ? pinnedLineageRepresentative(group) : root,
        ),
      });
    }
  }
...
/** A drop into Active clears the moved lineage's pins from any source section.
 * Independently shelved subtrees and hidden terminal children retain their own pins. */
export function getThreadListV2DropUnpinTargets(
  input: ThreadListV2LifecycleScope & {
    readonly threads: readonly EnvironmentThreadShell[];
    readonly thread: EnvironmentThreadShell;
  },
): readonly EnvironmentThreadShell[] {
  if (!canUseThreadListV2LifecycleActions(input.thread)) return [];
  const rootKey = threadShellKey(input.thread);
  const root = input.threads.find((thread) => threadShellKey(thread) === rootKey);
  if (root === undefined) return [];
  // Waking the root leaves independently snoozed descendant subtrees shelved.
  const awakenedRoot = { ...root, snoozedAt: null, snoozedUntil: null };
  const { activeGroups, pinnedGroups, snoozedGroups, settledGroups } = canonicalThreadListV2Groups({
    ...input,
    threads: input.threads.map((thread) => (thread === root ? awakenedRoot : thread)),
  });
  const group = [...activeGroups, ...pinnedGroups, ...snoozedGroups, ...settledGroups].find(
    (group) => threadShellKey(group[0]) === rootKey,
  );
  return (group ?? [])
    .filter((thread) => thread.pinnedAt != null)
    .map((thread) => (thread === awakenedRoot ? root : thread));
}

/** One ordering representative per card lineage, independent of search or disclosure. */
export function getThreadListV2OrderedSection(input: {
  ...
  const ordered =
    input.section === "pinned"
      ? sortPinnedThreadsByOrderKey(pinnedGroups.map(pinnedLineageRepresentative))
      : activeGroups.map((group) => group[0]);
  const pending =
...
export function buildThreadListV2Items(input: {
  ...
  const {
    ...
    snoozedCount,
    nextSnoozeWakeAt,
  } = partitionThreadListV2Lineage({ ...input, threads: searchedThreads, threadDepths });
