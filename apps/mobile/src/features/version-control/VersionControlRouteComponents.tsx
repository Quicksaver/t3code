  ...
  return (
    ...
    >
      <SymbolView
        name={props.icon}
        size={13}
        tintColorClassName={props.danger ? "accent-danger-foreground" : "accent-icon"}
        type="monochrome"
      />
      <Text
        className={cn(
            ...
            <View className="mt-4 gap-2">
              {props.remoteNames.map((remoteName) => (
                <View key={remoteName} className="overflow-hidden rounded-2xl">
                  <Pressable
                    accessibilityRole="button"
                    className="min-h-12 justify-center border border-border bg-subtle px-4 active:opacity-70"
                    disabled={props.disabled}
