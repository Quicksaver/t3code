...
export function archivedThreadTimestampValue(
  ...
  const value = field === "createdAt" ? thread.createdAt : thread.archivedAt;
  return value === null || Number.isNaN(Date.parse(value)) ? null : value;
}
...
function archivedThreadTimestamp(
  ...
  const value = archivedThreadTimestampValue(thread, field);
  return value === null ? null : Date.parse(value);
}

export function formatArchivedThreadRelativeTime(input: string | null): string | null {
  return input === null || Number.isNaN(Date.parse(input)) ? null : relativeTime(input);
}

function archivedTimestampOrder(
  direction: ArchivedThreadSortDirection,
): Order.Order<number | null> {
  const order = direction === "asc" ? Order.Number : Order.flip(Order.Number);
...
export function compareArchivedThreads(
  ...
  const timestampComparison = archivedTimestampOrder(sort.direction)(leftTimestamp, rightTimestamp);
...
export function buildArchivedThreadGroups(input: {
  ...
  return Arr.sort(
    ...
    Order.mapInput(
      Order.Struct({
        timestamp: archivedTimestampOrder(input.sort.direction),
        title: Order.String,
        key: Order.String,
      }),
