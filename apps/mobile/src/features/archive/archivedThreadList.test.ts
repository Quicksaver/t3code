...
describe("buildArchivedThreadGroups", () => {
  ...
  it("keeps invalid archive dates absent and after valid dates", () => {
    const project = makeProject({ id: ProjectId.make("project-1"), title: "T3 Code" });
    ...
    expect(archivedThreadTimestampValue(invalidArchivedAt, "archivedAt")).toBeNull();
  });
...
describe("formatArchivedThreadRelativeTime", () => {
  ...
  it("sorts invalid archive timestamps after valid timestamps in either direction", () => {
    ...
    const newest = buildArchivedThreadGroups({
      snapshots,
      environmentId: null,
      search: parseArchivedThreadSearchInput(""),
